...
def make_bar_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.bar([i - width / 2 for i in x], plot_df["comparison_delta_dw"], width=width, label="Non-mass contrast: ΔDw (dimensionless)")
    ax.bar([i + width / 2 for i in x], plot_df["delta_mv_shell_minus_bg"], width=width, label="Mass-volume contrast: ΔMVdens (proxy units)")
    ax.axhline(0.0, linewidth=1)
    ax.set_xticks(list(x))
    ax.set_xticklabels(plot_df["wise_name"], rotation=25, ha="right")
    ax.set_ylabel("Difference value")
    ax.set_title("Comparison of non-mass structural contrast and mass-volume contrast")
    ax.legend()
    fig.tight_layout()
...
def make_scatter_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    label_offsets = {
        "G000.649-00.080": (-45, 6),
        "G012.820-00.238": (-45, 6),
        "G233.830-00.180": (-1, -13),
        "G345.346-00.900": (-45, 6),
        "G351.311+00.663": (-70, 6),
    }
    ...
    for _, row in df.iterrows():
        ...
        ax.scatter([x], [y], s=70)
        offset = label_offsets.get(row["wise_name"], (8, 6))
        ax.annotate(row["wise_name"], (x, y), xytext=offset, textcoords="offset points")

    ax.set_xlabel("ΔDw = Dw_shell - Dw_bg (dimensionless)")
    ax.set_ylabel("ΔMVdens = MVdens_shell - MVdens_bg (proxy units)")
    ax.set_title("Comparison map: aligned, branching, and bridge-amplified cases")
    fig.tight_layout()
