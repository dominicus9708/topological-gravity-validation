...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lambda-edge", type=float, default=0.5, help="Edge-fraction correction strength for r_topo = r_eff * (1 - lambda_edge * edge_fraction)")
...
def add_topological_metrics(df: pd.DataFrame, rho_bg: float, gamma: float, lambda_edge: float) -> pd.DataFrame:
    out = df.copy()
    ...
    # Recompute everything from official input only.
    out["effective_volume_proxy_standard_like"] = (4.0 / 3.0) * np.pi * (out["radius_eff"] ** 3)
    out["rho_proxy_standard_like"] = np.where(
        out["effective_volume_proxy_standard_like"] > 0,
        out["mass_preferred_sum"] / out["effective_volume_proxy_standard_like"],
        np.nan,
    )

    edge_used = out["edge_fraction"].fillna(0.0).clip(lower=0.0, upper=1.0)
    out["r_topo"] = out["radius_eff"] * (1.0 - lambda_edge * edge_used)
    out["r_topo"] = out["r_topo"].clip(lower=1e-9)

    out["V_topo"] = (4.0 / 3.0) * np.pi * (out["r_topo"] ** 3)
    out["rho_proxy_topo"] = np.where(
        out["V_topo"] > 0,
        out["mass_preferred_sum"] / out["V_topo"],
        np.nan,
    )
...
def main() -> int:
    ...
    master_topo = add_topological_metrics(master, rho_bg=rho_bg, gamma=args.gamma, lambda_edge=args.lambda_edge)
    core_rep_topo = add_topological_metrics(core_rep, rho_bg=rho_bg, gamma=args.gamma, lambda_edge=args.lambda_edge)
    support_rep_topo = add_topological_metrics(support_rep, rho_bg=rho_bg, gamma=args.gamma, lambda_edge=args.lambda_edge)
    core_stacked_topo = add_topological_metrics(core_stacked, rho_bg=rho_bg, gamma=args.gamma, lambda_edge=args.lambda_edge)
    support_stacked_topo = add_topological_metrics(support_stacked, rho_bg=rho_bg, gamma=args.gamma, lambda_edge=args.lambda_edge)
    ...
    if len(master_topo):
        if master_topo["radius_eff"].notna().any() and master_topo["sigma_void"].notna().any():
            save_scatter(
                ...
                "Topological baseline 002: radius vs sigma_void",
        ...
        if master_topo["radius_eff"].notna().any() and master_topo["D_void"].notna().any():
            save_scatter(
                ...
                "Topological baseline 002: radius vs D_void",
                "radius_eff", "D_void",
        ...
        if master_topo["radius_eff"].notna().any() and master_topo["r_topo"].notna().any():
            save_scatter(
                ...
                "Topological baseline 002: radius_eff vs r_topo",
                "radius_eff", "r_topo",
                run_dir / "topological_radius_eff_vs_r_topo.png",
            )
        if "algorithm" in master_topo.columns:
            save_bar_counts(master_topo, "algorithm", "Topological baseline 002: algorithm counts", run_dir / "topological_algorithm_counts.png")
        if core_rep_topo["D_void"].notna().any():
            save_rank_plot(
                core_rep_topo, "D_void",
                "Core representatives 002: D_void",
                "D_void",
        ...
        if core_stacked_topo["sigma_void"].notna().any():
            save_rank_plot(
                ...
                "Core stacked 002: sigma_void",
                "sigma_void",
                run_dir / "topological_core_stacked_sigma_void.png",
            )

    lines = []
    lines.append("Cosmic Void Structural Validation topological summary (002 edge-corrected radius)")
    lines.append("========================================================================")
    ...
    lines.append(
        ...
        "void structural contrast and effective structural dimension using an edge-corrected derived radius. "
    ...
    lines.append(f"lambda_edge = {args.lambda_edge}")
    ...
    lines.append("-------------------")
    lines.append(
        "This stage uses the same official input source as the standard stage, but recomputes all derived quantities "
        "internally and replaces the catalog radius with an edge-corrected topological radius before evaluating "
        "sigma_void and D_void."
    )
