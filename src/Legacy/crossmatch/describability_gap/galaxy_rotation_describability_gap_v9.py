...
class ModelConfig:
    ...
    tag_name: str = "describability_gap_v9"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Integrated state-history describability-gap pipeline for galaxy rotation data."
    )
    ...
    parser.add_argument("--tag", default="describability_gap_v9", help="Tag appended to timestamp folder name.")
...
def load_config(config_json: Optional[str], segment_count_override: Optional[int], tag_override: Optional[str]) -> ModelConfig:
    ...
    config.state_weight = float(np.clip(config.state_weight, 0.0, 1.0))
    return config
...
def build_segment_table(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    # state axis from current structural state
    state_raw = np.abs(Dw - config.D_star)
    d0 = config.state_scale_factor * positive_median(state_raw, fallback=0.1)
    state_term = sat_unit(state_raw, d0)

    # history axis from smoothed derivative + causal propagation
    Dw_smooth = rolling_mean(Dw, config.dw_smoothing_window)
    dDw_dr = np.gradient(Dw_smooth, r_mid)
    grad_raw = np.abs(dDw_dr)
    g0 = config.grad_scale_factor * positive_median(grad_raw, fallback=0.1)
    source_term = sat_unit(grad_raw, g0)

    seg["state_term_i"] = state_term
    ...
    history_term = P_raw / (P_raw + max(p0, DEFAULT_EPS))

    seg["propagation_term_i"] = P_raw
    seg["history_term_i"] = history_term

    xi = config.state_weight * state_term + (1.0 - config.state_weight) * history_term
    delta = config.delta_floor + (config.delta_ceiling - config.delta_floor) * np.clip(xi, 0.0, 1.0)
    seg["delta_struct_i"] = delta
...
def summarize_galaxy(point_df: pd.DataFrame, seg_df: pd.DataFrame) -> Dict[str, float]:
    ...
    return {
        ...
        "mean_state_term_i": float(seg_df["state_term_i"].mean()),
        ...
        "history_term_mean": float(np.mean(point_df["history_term_i"].to_numpy(dtype=float))),
...
def plot_one_galaxy(point_df: pd.DataFrame, out_path: Path, dpi: int) -> None:
    ...
    ax.plot(x, d["state_term_i"].to_numpy(dtype=float), linewidth=1.2, label="state term")
    ax.set_ylabel("State axis")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")
    ...
    ax.plot(x, d["history_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="history term")
    ax.set_ylabel("History axis")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v9.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v9.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v9.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "v9 integrates the v3 state axis and the v8 history axis. "
            "Describability loss is modeled as a minimal weighted combination of present structural state "
            "(distance from D_star) and causally accumulated rearrangement history from smoothed dDw/dr."
        ),
        "minimal_story": {
            "D = D_bg + delta_D_gal": "galaxy as local structural deviation over a background field",
            "state_term_i": "present local structural state based on |Dw_i - D_star|",
            "history_source_i": "rearrangement source from smoothed radial gradient of Dw",
            "history_term_i": "causally accumulated outward history term",
            "DeltaD_i": "weighted combination of state axis and history axis",
        },
    }
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v9.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v9.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v9.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v9.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v9.csv", index=False)
    ...
    aggregate = {
        ...
        "mean_state_term_i": float(summary_df["mean_state_term_i"].mean()) if not summary_df.empty else float("nan"),
        "mean_history_term_mean": float(summary_df["history_term_mean"].mean()) if not summary_df.empty else float("nan"),
        "mean_causal_horizon_kpc": float(summary_df["causal_horizon_kpc"].mean()) if not summary_df.empty else float("nan"),
    }
    (run_dir / "aggregate_metrics_v9.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
