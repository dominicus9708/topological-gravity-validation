...
class ModelConfig:
    ...
    # v5: outward cumulative propagation driven eta
    eta_floor: float = 0.30
    eta_ceiling: float = 0.98
    eta_state_weight: float = 0.35
    eta_transition_weight: float = 0.15
    eta_propagation_weight: float = 0.50
    eta_transition_sharpness: float = 5.0
    eta_center: float = 0.52
    propagation_alpha: float = 1.0
    propagation_beta: float = 1.0

    tag_name: str = "describability_gap_v5"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="describability_gap_v5", help="Tag appended to timestamp folder name.")
...
def local_transition(arr: np.ndarray) -> np.ndarray:
    arr = np.asarray(arr, dtype=float)
    n = arr.size
    ...
    for i in range(n):
        ...
        out[i] = float(np.mean(diffs)) if diffs else 0.0
    return out


def logistic_shaped(x: np.ndarray, sharpness: float, center: float) -> np.ndarray:
...
def derive_external_projection(seg: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    ...
    Dw_dev = np.abs(out["Dw_i"].to_numpy(dtype=float) - config.D_star)
    rho_log = np.log1p(np.clip(out["segment_density_eff_msun_per_kpc3"].to_numpy(dtype=float), 0.0, None))
    S_local = np.clip(out["S_i"].to_numpy(dtype=float), 0.0, None)
    I_int = np.clip(np.abs(out["I_int_i"].to_numpy(dtype=float)), 0.0, None)

    sigma_term = sat_unit(sigma, positive_median(sigma))
    dw_term = sat_unit(Dw_dev, positive_median(Dw_dev, fallback=0.25 * (config.dw_max - config.dw_min)))
    density_term = sat_unit(rho_log, positive_median(rho_log))
    flow_term = sat_unit(np.log1p(I_int), positive_median(np.log1p(I_int)))

    state_combo = 0.34 * sigma_term + 0.28 * dw_term + 0.20 * density_term + 0.18 * flow_term
    transition_raw = local_transition(state_combo)
    transition_term = sat_unit(transition_raw, positive_median(transition_raw, fallback=0.1))

    # outward cumulative propagation: each outer shell receives accumulated inner-shell structural propagation
    r_mid = out["r_mid_kpc"].to_numpy(dtype=float)
    dr = np.clip(out["dr_kpc"].to_numpy(dtype=float), DEFAULT_EPS, None)
    r_max = max(float(np.nanmax(r_mid)), DEFAULT_EPS)
    source_term = np.power(np.clip(sigma_term * dw_term * density_term, 0.0, None), config.propagation_alpha) * np.power(np.clip(flow_term + sat_unit(S_local, positive_median(S_local, fallback=1.0)), 0.0, None), config.propagation_beta)

    P_raw = np.zeros_like(source_term)
    for i in range(len(source_term)):
        total = 0.0
        for j in range(i + 1):
            radial_gap = max(r_mid[i] - r_mid[j], 0.0)
            attenuation = math.exp(-radial_gap / max(float(out["ell_info_kpc"].iloc[0]), DEFAULT_EPS))
            shell_weight = dr[j] / max(r_max, DEFAULT_EPS)
            total += source_term[j] * attenuation * shell_weight
        P_raw[i] = total
    propagation_term = sat_unit(P_raw, positive_median(P_raw, fallback=0.1))

    raw = (
        config.eta_state_weight * state_combo
        + config.eta_transition_weight * transition_term
        + config.eta_propagation_weight * propagation_term
    )
    raw = np.clip(raw, 0.0, 1.0)
    shaped = logistic_shaped(raw, config.eta_transition_sharpness, config.eta_center)
    ...
    out["eta_density_term_i"] = density_term
    out["eta_flow_term_i"] = flow_term
    out["eta_transition_term_i"] = transition_term
    out["propagation_source_i"] = source_term
    out["propagation_term_i"] = propagation_term
    out["eta_obs_i"] = eta
    out["I_ext_i"] = eta * out["I_int_i"].to_numpy(dtype=float)
    out["DeltaD_i"] = np.abs(out["I_int_i"].to_numpy(dtype=float) - out["I_ext_i"].to_numpy(dtype=float)) / np.clip(np.abs(out["I_int_i"].to_numpy(dtype=float)), DEFAULT_EPS, None)
    return out
...
def build_segment_table(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    ell_info = float(np.clip(ell_nominal, config.ell_min_factor * dr_med, max(config.ell_max_fraction_of_rmax * r_max, config.ell_min_factor * dr_med + DEFAULT_EPS)))
...
def plot_one_galaxy(point_df: pd.DataFrame, out_path: Path, dpi: int) -> None:
    ...
    ax.plot(x, d["eta_density_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="eta density term")
    ax.plot(x, d["eta_flow_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="eta flow term")
    ax.plot(x, d["eta_transition_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="eta transition term")
    ax.plot(x, d["propagation_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="propagation term")
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v5.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v5.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v5.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "Baryonic Newton baseline preserved. v5 derives external describability from an outward cumulative propagation quantity. "
            "Each outer segment receives attenuated structural propagation accumulated from all inner segments."
        ),
        "derived_projection": {
            "state_combo": "0.34*sigma + 0.28*Dw_dev + 0.20*density + 0.18*flow (all saturating-normalized)",
            "transition_term": "local transition of state_combo",
            "propagation_source_i": "(sigma_term*dw_term*density_term)^alpha * (flow_term + sat(S_i))^beta",
            "propagation_term_i": "outward cumulative attenuated sum of propagation_source_i",
            "eta_obs_i": "eta_floor + (eta_ceiling - eta_floor) * logistic(state + transition + propagation)",
            "pointwise_mode": "linear interpolation from segment midpoints",
        },
    }
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v5.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v5.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v5.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v5.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v5.csv", index=False)
