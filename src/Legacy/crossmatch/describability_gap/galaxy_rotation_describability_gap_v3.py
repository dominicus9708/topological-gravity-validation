...
class ModelConfig:
    ...
    # derived external describability projection (absolute structural-state driven)
    eta_floor: float = 0.30
    eta_ceiling: float = 0.98
    eta_sigma_weight: float = 0.30
    eta_dw_weight: float = 0.25
    eta_density_weight: float = 0.25
    eta_flow_weight: float = 0.20
    eta_transition_sharpness: float = 5.0
    eta_center: float = 0.55

    tag_name: str = "describability_gap_v3"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="describability_gap_v3", help="Tag appended to timestamp folder name.")
...
def minmax_unit(arr: np.ndarray) -> np.ndarray:
...
def derive_external_projection(seg: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    ...
    rho = out["segment_density_eff_msun_per_kpc3"].to_numpy(dtype=float)
    I_int = out["I_int_i"].to_numpy(dtype=float)

    sigma_term = minmax_unit(sigma)
    dw_term = minmax_unit(np.abs(Dw - config.D_star))
    dens_term = minmax_unit(np.log1p(np.clip(rho, 0.0, None)))
    flow_term = minmax_unit(np.log1p(np.clip(np.abs(I_int), 0.0, None)))

    raw = (
        config.eta_sigma_weight * sigma_term
        + config.eta_dw_weight * dw_term
        + config.eta_density_weight * dens_term
    ...
    out["eta_density_term_i"] = dens_term
...
def evaluate_pointwise(df: pd.DataFrame, seg: pd.DataFrame) -> pd.DataFrame:
    ...
    for s in seg.itertuples():
        ...
        local["eta_density_term_i"] = float(s.eta_density_term_i)
        local["eta_flow_term_i"] = float(s.eta_flow_term_i)
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v3.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v3.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v3.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "Baryonic Newton baseline preserved. External describability is derived from absolute structural-state quantities, "
            "not from an explicit radius baseline. eta_obs_i is induced from sigma_i, |Dw_i-D_star|, effective density, and internal structural flow strength."
        ),
        "derived_projection": {
            "eta_sigma_term_i": "minmax(sigma_i)",
            "eta_dw_term_i": "minmax(|Dw_i - D_star|)",
            "eta_density_term_i": "minmax(log(1 + rho_eff_i))",
            "eta_flow_term_i": "minmax(log(1 + |I_int_i|))",
            "eta_obs_i": "eta_floor + (eta_ceiling - eta_floor) * logistic(weighted_sum - eta_center)",
        },
    }
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v3.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v3.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v3.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v3.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v3.csv", index=False)
    ...
    (run_dir / "aggregate_metrics_v3.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
