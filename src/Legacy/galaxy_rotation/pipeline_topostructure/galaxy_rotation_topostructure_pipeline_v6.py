...
# V6 concept
#
# 1) Compute a segment-wise local scaling exponent alpha_i from observable shell
#    structure rather than using a density-only structural proxy.
# 2) Construct effective structural dimension D_i from alpha_i.
# 3) Split structural deviation into:
#       sigma0_i : deviation from galaxy-level mean structure
#       sigma1_i : inter-segment structural deviation / local structural gradient
# 4) Build the coupling beta_i from:
#       - dynamical persistence inspired by  d_t alpha + v.grad alpha = -kappa(alpha-alpha*)
#       - statistical stability weight P ~ exp(-E / T_eff)
# 5) Use a bounded, independent, nonnegative translation term at acceleration
#    level so the model is not glued to baryon-only while avoiding V5 over-shoot.
#
# This file keeps the same timestamped output / per_galaxy / merged / plots
...
class ModelConfig:
    ...
    # Coupling and structure visibility
    beta0: float = 2.5
    ...
    # Dynamical persistence term  |v·∇D| / (kappa0 + kappa1 |v·∇D|)
    kappa0: float = 0.08
    kappa1: float = 1.0

    # Statistical stability term exp(-E / T_eff)
    t_eff: float = 0.15
    c_delta: float = 1.0
    c_grad: float = 1.0

    # Independent translation term
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v6 with local scaling exponent and statistical-dynamical coupling."
    )
    ...
    parser.add_argument("--tag", default="topostructure_v6", help="Optional tag to append after timestamp folder name.")
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, thickness: Optional[GalaxyThickness], config: ModelConfig):
    ...
    # First pass: shell geometry and shell mass
    ...
    # Local scaling exponent alpha_i from shell scaling between neighboring segments
    ...
    # Structural deviations: absolute and inter-segment
    ...
    # Dynamical persistence inspired by  d_t alpha + v.grad alpha = -kappa(alpha-alpha*)
    # Use |v * grad D| / (kappa0 + kappa1 |v * grad D|)
    v_norm = np.clip(seg_table["v_bar_internal_segment_kms"] / np.maximum(seg_table["v_bar_internal_segment_kms"].max(), DEFAULT_EPS), 0.0, None)
    gradD_abs = np.abs(seg_table["sigma1_i"])
    vdgrad = v_norm * gradD_abs
    seg_table["vdgrad_i"] = vdgrad
    seg_table["dyn_persist_i"] = vdgrad / (config.kappa0 + config.kappa1 * vdgrad + DEFAULT_EPS)

    # Statistical stability: P ~ exp(-E / T_eff)
    E_i = config.c_delta * np.square(seg_table["sigma0_i"]) + config.c_grad * np.square(seg_table["sigma1_i"])
    seg_table["e_struct_i"] = E_i
    seg_table["p_stat_i"] = np.exp(-E_i / max(config.t_eff, DEFAULT_EPS))

    # Coupling strength beta_i from dynamical persistence and statistical stability
    seg_table["beta_i"] = config.beta0 * seg_table["dyn_persist_i"] * seg_table["p_stat_i"]

    # Independent, bounded, nonnegative translation term
    ...
    delta_mag = np.abs(np.tanh(config.alpha_clip * seg_table["sigma0_i"])) * np.abs(np.tanh(config.alpha_clip * seg_table["sigma1_i"]))
    ...
    # Broadcast back to pointwise rows
    for _, seg in seg_table.iterrows():
        ...
        local["vdgrad_i"] = float(seg["vdgrad_i"])
        local["dyn_persist_i"] = float(seg["dyn_persist_i"])
        local["e_struct_i"] = float(seg["e_struct_i"])
        local["p_stat_i"] = float(seg["p_stat_i"])
    ...
    metrics = {
        ...
        "mean_p_stat_i": float(seg_table["p_stat_i"].mean()),
        "mean_dyn_persist_i": float(seg_table["dyn_persist_i"].mean()),
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    fig.suptitle(f"{galaxy} Topostructure V6 Diagnostics", fontsize=14)
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        pd.concat(pointwise_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_topostructure_v6.csv", index=False)
    if segment_frames:
        pd.concat(segment_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_topostructure_v6.csv", index=False)
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_desc_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(merged_dir / "merged_summary_topostructure_v6.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "dpi": args.dpi,
        "model_note": "V6 uses local scaling exponent alpha_i, dual structural deviation sigma0/sigma1, dynamical persistence, and statistical stability for beta_i.",
    }
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure_v6.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure_v6.csv"
            plot_path = plots_dir / f"{slug}_topostructure_v6_diagnostics.png"
    ...
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_desc_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(run_dir / "topostructure_summary_v6.csv", index=False)
        aggregate = {
            ...
            "mean_p_stat_i": float(summary_df["mean_p_stat_i"].mean()),
            "mean_dyn_persist_i": float(summary_df["mean_dyn_persist_i"].mean()),
            "mean_g_trans": float(summary_df["mean_g_trans"].mean()),
        }
        (run_dir / "aggregate_metrics_v6.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures_v6.csv", index=False)
