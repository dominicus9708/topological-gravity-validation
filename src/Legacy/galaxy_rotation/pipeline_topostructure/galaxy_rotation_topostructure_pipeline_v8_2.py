...
# V8.2 concept
#
# Keep the user's information-gravity interpretation:
#   - structural source is NOT baryonic mass
#   - cumulative source is an informational gravitational source
#   - G_topo is an information-source -> acceleration conversion factor
#
# Main repair relative to V8/V8.1:
#   Instead of using Dw^2 as the source, use structural perturbation itself.
#
#   Dw[i]          = 2 + ln(1 + h[i]/r[i]) / ln(r[i] / (r_min/10))
#   sigma[i]       = |Dw[i] - Dw[i-1]|
#   sigma_scale    = 1 / ln(r_max/r_min)
#   beta[i]        = exp(-sigma[i] / sigma_scale)
#
# Informational source density candidates:
#   source[i] = sigma[i]^p * beta[i]
#      or     = |Dw[i]-2|^p * beta[i]
#
# Default in this file uses sigma-based source, which better matches the idea
# that topological gravity emerges from structural mismatch / perturbation,
# not from the baseline structure itself.
# -----------------------------------------------------------------------------
...
class ModelConfig:
    ...
    # informational gravity normalization
    g_topo_factor: float = G_TOPO_DEFAULT

    # source model
    source_mode: str = "sigma"   # sigma, dw_minus_2, hybrid
    source_power: float = 2.0
    hybrid_weight_sigma: float = 1.0
    hybrid_weight_dw: float = 1.0

    # optional cumulative attenuation to prevent runaway accumulation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v8.2 using perturbation-based informational gravity source."
    )
    ...
    parser.add_argument("--tag", default="topostructure_v8_2", help="Optional tag to append after timestamp folder name.")
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, config: ModelConfig):
    ...
    for i in range(len(seg_table)):
        if i == 0:
            ...
            sigma_vals.append(abs(dw_vals[i] - dw_vals[i - 1]))
    seg_table["sigma_i"] = sigma_vals
    seg_table["beta_i"] = np.exp(-seg_table["sigma_i"] / max(sigma_scale, DEFAULT_EPS))

    # informational perturbation source
    if config.source_mode == "sigma":
        source_core = np.power(np.abs(seg_table["sigma_i"]), config.source_power)
    elif config.source_mode == "dw_minus_2":
        source_core = np.power(np.abs(seg_table["Dw_i"] - 2.0), config.source_power)
    elif config.source_mode == "hybrid":
        source_core = (
            config.hybrid_weight_sigma * np.power(np.abs(seg_table["sigma_i"]), config.source_power)
            + config.hybrid_weight_dw * np.power(np.abs(seg_table["Dw_i"] - 2.0), config.source_power)
        )
    else:
    ...
    seg_table["g_topo_km2s2_per_kpc"] = config.g_topo_factor * seg_table["I_struct_i"] / np.clip(np.square(seg_table["r_mid_kpc"]), DEFAULT_EPS, None)
    ...
    for _, seg in seg_table.iterrows():
        ...
        local["source_core_i"] = float(seg["source_core_i"])
        local["info_source_i"] = float(seg["info_source_i"])
        local["I_struct_i"] = float(seg["I_struct_i"])
        local["g_topo_km2s2_per_kpc"] = np.clip(config.g_topo_factor * float(seg["I_struct_i"]) / np.clip(np.square(local["r_kpc"]), DEFAULT_EPS, None), 0.0, None)
        ...
        local["sigma_scale"] = float(seg["sigma_scale"])
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    fig.suptitle(f"{galaxy} Topostructure V8.2 Diagnostics", fontsize=14)
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        pd.concat(pointwise_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_topostructure_v8_2.csv", index=False)
    if segment_frames:
        pd.concat(segment_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_topostructure_v8_2.csv", index=False)
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(merged_dir / "merged_summary_topostructure_v8_2.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": "V8.2 preserves informational gravity interpretation and uses perturbation-based source (default sigma^p * beta) instead of Dw^2 * beta.",
    }
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure_v8_2.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure_v8_2.csv"
            plot_path = plots_dir / f"{slug}_topostructure_v8_2_diagnostics.png"
    ...
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(run_dir / "topostructure_summary_v8_2.csv", index=False)
        aggregate = {
            ...
            "mean_sigma_i": float(summary_df["mean_sigma_i"].mean()),
            "mean_beta_i": float(summary_df["mean_beta_i"].mean()),
            "mean_info_source_i": float(summary_df["mean_info_source_i"].mean()),
            "mean_v_struct_kms": float(summary_df["mean_v_struct_kms"].mean()),
        }
        (run_dir / "aggregate_metrics_v8_2.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures_v8_2.csv", index=False)
