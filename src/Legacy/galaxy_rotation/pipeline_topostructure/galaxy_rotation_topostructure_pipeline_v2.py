...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Segment-based topological structure pipeline for enriched galaxy CSV files.")
    parser.add_argument("--input-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--thickness-csv", default=None)
    parser.add_argument("--glob", default="*.csv")
    parser.add_argument("--config-json", default=None)
    parser.add_argument("--segment-count", type=int, default=None)
    parser.add_argument("--galaxy-filter", nargs="*", default=None)
...
def prepare_dataframe(file_path: Path) -> Tuple[pd.DataFrame, str]:
    ...
    if v_bar_col is None:
        ...
        missing_components = [
            name
            for name, col in [("v_gas", v_gas_col), ("v_disk", v_disk_col), ("v_bulge", v_bul_col)]
            if col is None
        ]
        if missing_components:
            raise ValueError(
                "Could not determine baryonic rotation column. Missing components: " + ", ".join(missing_components)
            )
...
def write_run_metadata(output_dir: Path, config: ModelConfig, args: argparse.Namespace) -> None:
    payload = {"config": asdict(config), "input_dir": str(args.input_dir), "glob": args.glob, "thickness_csv": args.thickness_csv, "galaxy_filter": args.galaxy_filter}
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_table.to_csv(output_dir / f"{slug}_segment_structure.csv", index=False)
            point_table.to_csv(output_dir / f"{slug}_pointwise_topostructure.csv", index=False)
