...
def cumulative_with_attenuation(source: np.ndarray, dr: np.ndarray, radius: np.ndarray, attenuation_lambda: float) -> np.ndarray:
    ...
    base_lambda = max(attenuation_lambda, 0.0)
    out = np.zeros_like(source, dtype=float)
    out[0] = max(float(source[0] * dr[0]), 0.0)
    for i in range(1, len(source)):
        gap = max(radius[i] - radius[i - 1], 0.0)
        local_decay = math.exp(-base_lambda * gap) if base_lambda > 0 else 1.0
        propagated = out[i - 1] * local_decay
        out[i] = max(float(source[i] * dr[i]), 0.0) + propagated
    return out
...
def build_segments_base(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    source_core = mass_norm * (seg_table["sigma_i"] + seg_table["alpha_relax_i"])
