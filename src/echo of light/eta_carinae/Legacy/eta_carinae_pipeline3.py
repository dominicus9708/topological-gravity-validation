...
def get_component_draw_specs(mass_df: pd.DataFrame) -> list[dict]:
    ...
    if hom is not None:
        ...
        if np.isnan(major): major = 20.0
        if np.isnan(minor): minor = 10.0
    ...
    if out is not None:
        specs.append({'component_label':'outer_ejecta','shape':'circle','radius':6.2,'center':(0.0,0.0),'annotation_xy':(6.6,0.0),'annotation_text':'outer ejecta','knots':[(4.8,1.4),(-4.1,2.6),(2.4,-4.9),(-5.0,-1.3)]})
    return specs
...
def finalize_circular_axes(ax, title: str, info_lines: list[str]) -> None:
    ...
    ax.set_xlabel('arcsec'); ax.set_ylabel('arcsec'); ax.set_title(title)
...
def derive_phase_rank(label: str, phase_reference: str) -> float:
    ...
    if 'discovery' in text: return 0.0
    if 'early' in text: return 1.0
    if 'bulk' in text or 'plateau' in text: return 2.0
    if 'fast' in text or 'tail' in text: return 3.0
    return 0.0
...
def derive_phase_year(row: pd.Series) -> float:
    ...
    if '1850' in event_text or '1850' in phase_reference:
        if 'early' in label.lower(): return 1851.0
        if 'bulk' in label.lower(): return 1854.0
        if 'fast' in label.lower(): return 1858.0
        return 1854.0
...
def reconstruct_standard_timeseries(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out['phase_rank'] = [derive_phase_rank(str(l), str(p)) for l,p in zip(out['observation_label'], out['phase_reference'])]
    ...
    for _, row in out.iterrows():
        ...
        vel.append(float(np.mean(vals)) if vals else np.nan)
    out['velocity_magnitude_proxy_kmps'] = vel
...
def plot_standard_velocity(df: pd.DataFrame, output_png: Path) -> None:
    ...
    x = np.arange(len(df)); labels = df['observation_label'].astype(str).tolist()
    offsets = [-0.2,0.0,0.2]; width = 0.18
    ...
    plt.ylabel('Velocity (km/s)'); plt.title('Eta Carinae light echo velocity overview'); plt.legend(); plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def plot_standard_temperature_mass(df: pd.DataFrame, output_png: Path) -> None:
    ...
    ax1 = plt.gca()
    if temp.notna().any():
        ax1.plot(x, temp, marker='o', label='effective_temperature_k'); ax1.set_ylabel('Temperature (K)')
    ax1.set_xticks(x); ax1.set_xticklabels(labels, rotation=30, ha='right'); ax1.set_title('Eta Carinae observation context')
    if mass.notna().any():
        ax2 = ax1.twinx(); ax2.bar(x, mass.fillna(0.0), alpha=0.3, label='mass_context_msun_lower_bound'); ax2.set_ylabel('Mass context lower bound (Msun)')
    plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def plot_standard_timeseries_reconstructed(df: pd.DataFrame, output_png: Path) -> None:
    ...
    plt.xlabel('Reconstructed event-phase year'); plt.ylabel('Velocity magnitude proxy (km/s)'); plt.title('Eta Carinae reconstructed light echo phase timeline'); plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()


def plot_standard_circular_context(mass_df: pd.DataFrame, output_png: Path) -> None:
    fig, ax = plt.subplots(figsize=(9,9)); ax.set_aspect('equal', adjustable='box')
    add_ring_guides(ax); legend_handles = add_center_system(ax)
    for spec in get_component_draw_specs(mass_df):
        if spec['shape'] == 'ellipse':
            ...
            ax.add_patch(patch); ax.text(*spec['annotation_xy'], spec['annotation_text'], ha='center', va='bottom', fontsize=10)
            ...
            ax.add_patch(patch); ax.text(*spec['annotation_xy'], spec['annotation_text'], ha='left', va='center', fontsize=9)
    ...
    dedup = {h.get_label():h for h in legend_handles}
    leg1 = ax.legend(handles=list(dedup.values()), loc='upper right', frameon=True); ax.add_artist(leg1)
    finalize_circular_axes(ax, 'Eta Carinae standard circular context', ['Angular guide rings','2", 4", 6", 8", 10"','','Drawing mode:','standard context schematic','not epoch-by-epoch echo map'])
    plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def run_standard_pipeline(observation_csv: Path, mass_csv: Optional[Path], output_dir: Path) -> dict:
    ...
    if mass_csv: files.append(mass_csv)
    ...
    pd.DataFrame({'row_count':[len(prepared)],'spectral_velocity_rows':[int(prepared['spectral_velocity_kmps'].notna().sum()) if 'spectral_velocity_kmps' in prepared else 0],'bulk_velocity_rows':[int(prepared['bulk_velocity_kmps'].notna().sum()) if 'bulk_velocity_kmps' in prepared else 0],'fast_component_rows':[int(prepared['fast_component_velocity_kmps'].notna().sum()) if 'fast_component_velocity_kmps' in prepared else 0],'temperature_rows':[int(prepared['effective_temperature_k'].notna().sum()) if 'effective_temperature_k' in prepared else 0],'circular_context_generated':[circular_generated]}).to_csv(output_dir / 'eta_carinae_standard_summary.csv', index=False, encoding='utf-8-sig')
...
def estimate_volume_proxy(row: pd.Series) -> float:
    ...
    if geometry == 'torus':
        R = max(a,b); r = min(b,c); return float(2.0 * math.pi**2 * max(R,0.0) * max(r,0.0)**2)
    if geometry in ('two_ellipsoids_sum','bipolar_lobes'):
        ellipsoid = (4.0/3.0)*math.pi*max(a,0.0)*max(b,0.0)*max(c,0.0); return float(2.0*ellipsoid)
...
def select_mass_proxy(row: pd.Series) -> float:
    for key in ['mass_value_msun','gas_mass_msun','dust_mass_msun']:
        v = parse_numeric_like(row.get(key))
        if not np.isnan(v):
            return v
    return np.nan
...
def is_real_component_row(row: pd.Series) -> bool:
    ...
    return not (label.endswith('_context') or 'context' in comp_type or verification == 'context_only')
...
def derive_component_embedding_dim(row: pd.Series) -> float:
    ...
    if any(x in geometry for x in ['bipolar','ellipsoid','shell','torus']):
...
def derive_d_bg(real_df: pd.DataFrame) -> float:
    ...
    return float(dims.median()) if not dims.empty else 3.0
...
def prepare_mass_table(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    def combine_sigma(row: pd.Series) -> float:
        if not bool(row.get('is_real_component', False)): return np.nan
        vals=[]; wts=[]
        for key in ['sigma_mass_volume','sigma_velocity','sigma_anisotropy']:
            val = row.get(key); wt = weights.get(key,0.0)
            if not pd.isna(val) and wt > 0:
                vals.append(float(val)); wts.append(float(wt))
        return float(sum(v*w for v,w in zip(vals,wts))/sum(wts)) if vals else np.nan
...
def plot_mass_profile(df: pd.DataFrame, output_png: Path) -> None:
    ...
    plt.figure(figsize=(10,6))
    labels = plot_df['component_label'].astype(str).tolist(); x = np.arange(len(plot_df))
    sigma = pd.to_numeric(plot_df['sigma_eta'], errors='coerce'); dw = pd.to_numeric(plot_df['Dw_trial_mass_volume'], errors='coerce')
    ax1 = plt.gca(); ax1.bar(x, sigma.fillna(0.0), alpha=0.7, label='sigma_eta'); ax1.set_ylabel('Sigma'); ax1.set_xticks(x); ax1.set_xticklabels(labels, rotation=30, ha='right'); ax1.set_title('Eta Carinae component sigma / Dw profile')
    ax2 = ax1.twinx(); ax2.plot(x, dw, marker='o', label='Dw_trial_mass_volume'); ax2.set_ylabel('Dw trial')
    plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def plot_mass_velocity_geometry(df: pd.DataFrame, output_png: Path) -> None:
    ...
    ax1 = plt.gca()
    if vel.notna().any(): ax1.plot(x, vel, marker='o', label='bulk_velocity_kmps')
    if major.notna().any(): ax1.plot(x, major, marker='s', label='angular_major_axis_arcsec')
    ax1.set_xticks(x); ax1.set_xticklabels(labels, rotation=30, ha='right'); ax1.set_ylabel('Velocity / angular extent proxy'); ax1.set_title('Eta Carinae component geometry context'); ax1.legend(loc='upper left')
    if dust.notna().any():
        ax2 = ax1.twinx(); ax2.bar(x, dust.fillna(0.0), alpha=0.25, label='dust_mass_msun'); ax2.set_ylabel('Dust mass (Msun)')
    plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def plot_topological_circular_context(df: pd.DataFrame, output_png: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(9,9)); ax.set_aspect('equal', adjustable='box')
    add_ring_guides(ax); legend_handles = add_center_system(ax)
    sigma_map = {str(r['component_label']): parse_numeric_like(r.get('sigma_eta')) for _,r in plot_df.iterrows()}
    dw_map = {str(r['component_label']): parse_numeric_like(r.get('Dw_trial_mass_volume')) for _,r in plot_df.iterrows()}
    for spec in get_component_draw_specs(plot_df):
        label = spec['component_label']; sigma = sigma_map.get(label, np.nan); dw = dw_map.get(label, np.nan)
        linewidth = 1.2 if np.isnan(sigma) else 1.2 + 2.4*float(sigma)
        ann = spec['annotation_text']
        suffix = []
        if not np.isnan(sigma): suffix.append(f'σ={sigma:.2f}')
        if not np.isnan(dw): suffix.append(f'Dw={dw:.2f}')
        if suffix: ann += '\n' + ' / '.join(suffix)
        if spec['shape']=='ellipse':
            patch = Ellipse(spec['center'], width=spec['width'], height=spec['height'], angle=spec['angle'], fill=False, linewidth=linewidth)
            ax.add_patch(patch); ax.text(spec['annotation_xy'][0], spec['annotation_xy'][1], ann, ha='center', va='bottom', fontsize=9)
            legend_handles.append(Line2D([0],[0], color='black', linewidth=linewidth, label='Topological Homunculus'))
        elif spec['shape']=='circle' and label=='dust_torus':
            patch = Circle(spec['center'], spec['radius'], fill=False, linewidth=linewidth, linestyle='--')
            ax.add_patch(patch); ax.text(spec['annotation_xy'][0], spec['annotation_xy'][1], ann, ha='left', va='center', fontsize=9)
            legend_handles.append(Line2D([0],[0], color='black', linestyle='--', linewidth=linewidth, label='Topological dust torus'))
        elif spec['shape']=='circle' and label=='outer_ejecta':
            patch = Circle(spec['center'], spec['radius'], fill=False, linewidth=linewidth, linestyle='-.')
            ax.add_patch(patch)
            for x,y in spec.get('knots', []): ax.add_patch(Circle((x,y), 0.14, fill=True, alpha=0.7))
            ax.text(spec['annotation_xy'][0], spec['annotation_xy'][1], ann, ha='left', va='center', fontsize=9)
            legend_handles.append(Line2D([0],[0], color='black', linestyle='-.', linewidth=linewidth, label='Topological outer ejecta'))
    dedup = {h.get_label():h for h in legend_handles}
    leg1 = ax.legend(handles=list(dedup.values()), loc='upper right', frameon=True); ax.add_artist(leg1)
    finalize_circular_axes(ax, 'Eta Carinae topological circular context', ['Same circular frame as standard','line width scales with sigma','','Comparison mode:','topological circular context','shared geometry / shared scale'])
    plt.tight_layout(); plt.savefig(output_png, dpi=220); plt.close()
...
def run_mass_pipeline(mass_csv: Path, source_registry_csv: Optional[Path], output_dir: Path) -> dict:
    ...
    processed_dir = output_dir / 'processed_input'
    files = [mass_csv]
    if source_registry_csv: files.append(source_registry_csv)
    copy_processed_inputs(files, processed_dir)
    ...
    pd.DataFrame({'row_count_all':[len(prepared)],'row_count_real_components':[len(calc_df)],'mass_proxy_rows':[int(calc_df['mass_proxy_raw'].notna().sum())],'volume_proxy_rows':[int(calc_df['volume_proxy_raw'].notna().sum())],'sigma_rows':[int(calc_df['sigma_eta'].notna().sum())],'dw_rows':[int(calc_df['Dw_trial_mass_volume'].notna().sum())],'d_bg_derived':[derivation['d_bg_derived']],'lambda_derived':[derivation['lambda_derived']],'weight_sigma_mass_volume':[derivation['weight_sigma_mass_volume']],'weight_sigma_velocity':[derivation['weight_sigma_velocity']],'weight_sigma_anisotropy':[derivation['weight_sigma_anisotropy']]}).to_csv(output_dir / 'eta_carinae_mass_summary.csv', index=False, encoding='utf-8-sig')
...
def main() -> None:
    ...
    script_path = Path(__file__).resolve(); repo_root = Path(args.repo_root).resolve() if args.repo_root else find_repo_root(script_path)
    ...
    if not args.run_standard and not args.run_mass:
        run_standard = True; run_mass = True
    else:
        run_standard = args.run_standard; run_mass = args.run_mass
    stamp = timestamp_label(); standard_result = None; mass_result = None
    if run_standard:
