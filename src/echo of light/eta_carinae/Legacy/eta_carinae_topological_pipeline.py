...
def draw_label_with_connector(ax, target_xy, text_xy, text, ha="left"):
    ax.annotate(
        ...
        va="center",
        fontsize=9,
        bbox=dict(boxstyle="round,pad=0.2", facecolor="white", alpha=0.88, edgecolor="0.4"),
        arrowprops=dict(arrowstyle="-", lw=0.85, color="0.35"),
    )
...
def prepare_mass_table(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    out["log_mass_volume_proxy"] = np.where(
        out["is_real_component"] & out["mass_proxy_raw"].notna() & out["volume_proxy_raw"].notna() & (out["mass_proxy_raw"] > 0) & (out["volume_proxy_raw"] > 0),
    ...
    out["interpretation_role"] = np.where(
        ~out["is_real_component"], "excluded_context",
        np.where(
            out["sigma_eta"] >= 0.70, "strong_core_or_major_structural_distortion",
...
def component_linewidth(style_value: float) -> float:
    if np.isnan(style_value):
        return 1.5
    return 1.6 + 2.2 * float(style_value)
...
def make_effective_line(ax, patch, zorder=4):
    patch.set_facecolor("none")
    patch.set_edgecolor("black")
    patch.set_path_effects([pe.Stroke(linewidth=patch.get_linewidth()+1.6, foreground="white"), pe.Normal()])
    patch.set_zorder(zorder)
    ax.add_patch(patch)


def style_values_text(name, sigma_mv, sigma_v, sigma_a, sigma_eta, dw):
    parts = [name]
    ...
    if not np.isnan(sigma_eta) and not np.isnan(dw):
        parts.append(f"σ={sigma_eta:.2f} / Dw={dw:.2f}")
    return "\n".join(parts)
...
def plot_topological_circular_context(df: pd.DataFrame, output_png: Path) -> None:
    ...
    for spec in get_component_draw_specs(df):
        ...
        if spec["component_label"] == "homunculus_main_lobes":
            ...
            linestyle = (0, (8, 2, 1.6, 2)) if (not np.isnan(sigma_a) and sigma_a >= 0.35) else "-"
            ...
            make_effective_line(ax, patch)
            label_text = style_values_text(spec["label_name"], sigma_mv, np.nan, sigma_a, sigma_eta, dw)
            draw_label_with_connector(ax, spec["target_xy"], spec["label_xy"], label_text, ha="center")
            ...
            driver = sigma_mv
            lw = component_linewidth(driver)
            linestyle = ":" if (not np.isnan(driver) and driver >= 0.35) else "--"
            patch = Circle(spec["center"], spec["radius"], fill=False, linewidth=lw, linestyle=linestyle)
            make_effective_line(ax, patch)
            label_text = style_values_text(spec["label_name"], sigma_mv, np.nan, np.nan, sigma_eta, dw)
            draw_label_with_connector(ax, spec["target_xy"], spec["label_xy"], label_text, ha="left")
            ...
            lw = component_linewidth(driver)
            linestyle = (0, (11, 3)) if (not np.isnan(driver) and driver >= 0.35) else "-."
            patch = Circle(spec["center"], spec["radius"], fill=False, linewidth=lw, linestyle=linestyle)
            make_effective_line(ax, patch)
            knot_radius = 0.14 if np.isnan(driver) else 0.14 + 0.10 * float(driver)
            for x,y in spec.get("knots", []):
                ax.add_patch(Circle((x,y), knot_radius, fill=True, alpha=0.72, color="tab:blue"))
            label_text = style_values_text(spec["label_name"], sigma_mv, sigma_v, sigma_a, sigma_eta, dw)
            draw_label_with_connector(ax, spec["target_xy"], spec["label_xy"], label_text, ha="left")
    ...
    info_lines = ['Same circular frame as standard','same weight family, clearer styles','','Comparison mode:','component-wise topological context','labels moved outside + white halo']
...
def write_proxy_rationale(prepared: pd.DataFrame, output_path: Path) -> None:
    lines = []
    lines.append("Eta Carinae topological pipeline proxy rationale")
    lines.append("=" * 48)
    lines.append("")
    lines.append("Purpose")
    lines.append("-------")
    lines.append("This file records why component-row restructuring was introduced.")
    lines.append("The goal is not to fit a preferred answer, but to make the same")
    lines.append("component row carry mass + volume + velocity so that a structural")
    lines.append("topological quantity can be computed at all.")
    lines.append("")
    lines.append("Rules applied")
    lines.append("-------------")
    lines.append("1. homunculus_main_lobes")
    lines.append("   - if direct mass is missing, link to Great Eruption lower-bound mass proxy.")
    lines.append("   - reason: the Homunculus is a principal eruption structure, so using the")
    lines.append("     event-scale lower bound is a structural lower-bound proxy, not a tuned fit.")
    lines.append("")
    lines.append("2. dust_torus")
    lines.append("   - if geometry/depth/velocity are missing, inject conservative torus proxies.")
    lines.append("   - reason: torus had mass information but could not form a calculable M/V row.")
    lines.append("")
    lines.append("3. outer_ejecta_context -> outer_ejecta_component")
    lines.append("   - promote context-only row into a calculable outer-ejecta component row.")
    lines.append("   - reason: outer ejecta is physically relevant for distortion comparison,")
    lines.append("     so excluding it would understate the outer structural contribution.")
    lines.append("")
    lines.append("Interpretation note")
    lines.append("-------------------")
    lines.append("These proxies should be treated as conservative structural bridges.")
    lines.append("They enable a first consistency-oriented test. They are not claims that")
    lines.append("the inserted values are final measured truths.")
    lines.append("")
    lines.append("Applied per-row flags")
    lines.append("---------------------")
...
def write_interpretation_summary(prepared: pd.DataFrame, derivation: dict, output_path: Path) -> None:
    ...
    lines = []
    lines.append("Eta Carinae topological interpretation summary")
    lines.append("=" * 48)
    lines.append("")
    lines.append(f"d_bg_derived = {derivation['d_bg_derived']:.6f}")
    lines.append(f"lambda_derived = {derivation['lambda_derived']:.6f}")
    lines.append(f"weight_sigma_mass_volume = {derivation['weight_sigma_mass_volume']:.6f}")
    lines.append(f"weight_sigma_velocity = {derivation['weight_sigma_velocity']:.6f}")
    lines.append(f"weight_sigma_anisotropy = {derivation['weight_sigma_anisotropy']:.6f}")
    lines.append("")
    ...
    lines.append("-------------")
    lines.append("High sigma means stronger structural deviation from the background trial dimension.")
    lines.append("Lower Dw means stronger effective reduction from the background structural trial.")
    lines.append("This file is intended for interpretation-layer review, not just plotting.")
    output_path.write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    interpretation_cols = [
        "component_label","proxy_rule_applied","mass_proxy_raw","volume_proxy_raw",
        "sigma_mass_volume","sigma_velocity","sigma_anisotropy","sigma_eta",
        "Dw_trial_mass_volume","interpretation_role"
    ]
