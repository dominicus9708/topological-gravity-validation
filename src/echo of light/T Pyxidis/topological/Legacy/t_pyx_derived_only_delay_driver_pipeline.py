...
DEFAULT_OUTPUT_ROOT = Path(
    r"results/Echo of Light/output/T Pyxidis/topological/derived_only_delay_driver"
)
...
def build_patch_trial_table(
    ...
    # external observer (Dw = 3) vs internal effective structure gap
    ...
    out["Dw_trial_derived_only"] = d_bg - lambda_sigma * out["sigma_total"]

    # ----- Derived-only delay driver -----
    ...
    out = add_family_gradient(out, "Dw_trial_derived_only", "grad_Dw_trial")

    # derived terms only
    out["driver_grad_mv_term"] = normalize_series_01(
        (3.0 - out["Dw_trial_derived_only"]).abs() + out["grad_sigma_mass_volume"].abs()
    )
    out["driver_grad_burst_term"] = normalize_series_01(out["grad_sigma_local_burst"].abs())
    out["driver_gap_term"] = normalize_series_01((3.0 - out["Dw_trial_derived_only"]).abs())

    out["delay_driver"] = normalize_series_01(
        out["driver_grad_mv_term"] + out["driver_grad_burst_term"] + out["driver_gap_term"]
    )
...
def save_circular_layout(df: pd.DataFrame, out_path: Path, title: str) -> None:
    ...
    sc = ax.scatter(df["angle_rad"], df["theta_arcsec"], c=df["Dw_trial_derived_only"], s=95)
    for _, row in df.iterrows():
        ax.text(
            row["angle_rad"],
            row["theta_arcsec"] + 0.18,
            f"{row['echo_patch']}\nDw={row['Dw_trial_derived_only']:.2f}",
            ha="center",
            va="center",
            fontsize=9,
        )
    ...
    plt.colorbar(sc, pad=0.1, label="Dw_trial_derived_only")
    ax.set_title(title)
    plt.tight_layout()
...
def build_summary(patch_df: pd.DataFrame, trial_df: pd.DataFrame, input_csv: Path, out_dir: Path, params: dict) -> dict:
    return {
        "pipeline_name": "T Pyxidis Derived-Only Delay Driver Pipeline",
        ...
        "parameters": params,
        "notes": [
            "driver_geom is removed.",
            "delay_driver uses only derived terms: grad_mv_term + grad_burst_term + gap_term.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="T Pyxidis derived-only delay driver pipeline")
    ...
    write_csv(trial_df, out_dir / "t_pyx_derived_only_delay_driver_patch_comparison.csv")

    save_circular_layout(
        trial_df,
        plots_dir / "t_pyx_derived_only_delay_driver_circular_layout.png",
        "T Pyxidis: circular patch layout (derived-only delay-driver model)",
    )
    save_family_delay_theta_points(
        trial_df,
        plots_dir / "t_pyx_derived_only_delay_driver_family_delay_theta.png",
        "T Pyxidis: family delay-theta comparison (derived-only delay-driver)",
    )
    save_components_bars(
        trial_df,
        plots_dir / "t_pyx_derived_only_delay_driver_components.png",
        "T Pyxidis: structural components (bars)",
    )
    save_delay_driver_points(
        trial_df,
        plots_dir / "t_pyx_derived_only_delay_driver_profile.png",
        "T Pyxidis: standard delay norm vs derived-only delay driver (points)",
    )
