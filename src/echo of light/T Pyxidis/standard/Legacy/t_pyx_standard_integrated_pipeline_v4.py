...
def add_binary_name_legend(fig):
    text = (
        ...
        "Left circle: White dwarf\n"
        "Right circle: Donor companion\n"
        "Center marker: COM at exact graph center\n"
        "Both circles are drawn as a compressed,\n"
        "partly overlapping pair centered on the COM.\n"
        "This is intentional: it shows both the binary\n"
        "nature and the fact that the true physical\n"
        "separation is unresolved on the echo-patch scale."
    )
    fig.text(
        0.77, 0.74, text,
...
def save_circular_layout(df: pd.DataFrame, out_path: Path) -> None:
    ...
    inset = fig.add_axes([0.418, 0.418, 0.164, 0.164])
    inset.set_aspect("equal")
    pos = binary_overlay_positions()

    from matplotlib.patches import Circle
    wd_circle = Circle((pos["wd_x"], pos["wd_y"]), pos["radius"], fill=False, linewidth=1.5, edgecolor="black")
    donor_circle = Circle((pos["donor_x"], pos["donor_y"]), pos["radius"], fill=False, linewidth=1.5, edgecolor="black")
    inset.add_patch(wd_circle)
    inset.add_patch(donor_circle)
    inset.scatter([pos["com_x"]], [pos["com_y"]], s=70, marker="+", color="black", zorder=6)
