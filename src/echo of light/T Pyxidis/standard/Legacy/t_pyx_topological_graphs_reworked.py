...
def run_pipeline(patch_csv: str | Path, output_root: str | Path, object_name: str, mode_name: str) -> Path:
    df = pd.read_csv(patch_csv, encoding="utf-8-sig")
    for col in ["delay_time_days", "theta_arcsec", "z_1e17_cm"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    # first topological comparison layer
    ...
    df["sigma_topological"] = 0.60 * df["z_abs_norm"] + 0.40 * (1.0 - df["theta_norm"])
    df["Dw_trial_topological"] = 3.0 - df["sigma_topological"]

    delay_min = df["delay_time_days"].min()
    delay_max = df["delay_time_days"].max()
    df["delay_topological_proxy_days"] = delay_min + df["sigma_topological"] * (delay_max - delay_min)
    ...
    df.to_csv(out_dir / "t_pyx_topological_patch_comparison.csv", index=False, encoding="utf-8-sig")

    # 1) Circular topological layout
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection="polar")
    ax.set_theta_zero_location("E")
    ax.set_theta_direction(1)
    sc = ax.scatter(df["angle_rad"], df["theta_arcsec"], c=df["Dw_trial_topological"], s=90)
    ...
    ax.set_title("T Pyxidis topological: circular patch layout")
    plt.tight_layout()
    plt.savefig(out_dir / "t_pyx_topological_circular_layout.png", dpi=180)
    plt.close()

    # 2) Delay-vs-theta comparison, standard vs topological proxy, family-separated
    ...
    ax.set_title("T Pyxidis: delay vs angular distance, standard vs topological")
    ax.set_xlabel("Delay time (days)")
    ax.set_ylabel("Angular distance θ (arcsec)")
    ax.legend(ncol=2, fontsize=8)
    plt.tight_layout()
    plt.savefig(out_dir / "t_pyx_topological_delay_theta_comparison.png", dpi=180)
    plt.close()

    summary = [
        "T Pyxidis topological comparison run",
        f"patch_csv={Path(patch_csv)}",
        f"output_dir={out_dir}",
        "",
        "Produced graphs:",
        "- circular patch layout",
        "- delay-vs-theta family-separated standard vs topological comparison",
        "",
        "Topological trial definition:",
        "- sigma_topological = 0.60 * z_abs_norm + 0.40 * (1 - theta_norm)",
        "- Dw_trial_topological = 3 - sigma_topological",
        "- delay_topological_proxy_days rescales sigma into the standard delay range",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run T Pyxidis topological comparison graphs.")
    parser.add_argument(
        "--patch-csv",
