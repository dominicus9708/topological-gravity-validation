...
class ModelConfig:
    ...
    # describability gap from cumulative propagation only
    delta_floor: float = 0.0
    delta_ceiling: float = 1.0
    delta_p0_scale: float = 1.0

    tag_name: str = "describability_gap_v7"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Occam-style derivative-propagation pipeline for describability-gap diagnostics on galaxy rotation data."
    )
    ...
    parser.add_argument("--tag", default="describability_gap_v7", help="Tag appended to timestamp folder name.")
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v7.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v7.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v7.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "Occam-style v7: baryonic Newton baseline plus effective structural dimension Dw, smoothed radial derivative |dDw/dr| as the structural rearrangement source, outward cumulative propagation P, and a saturating propagation-to-gap map."
        ),
        "minimal_story": {
            ...
            "P_i": "outward cumulative attenuated sum of |dDw/dr| source from inner shells",
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v7.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v7.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v7.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v7.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v7.csv", index=False)
