...
class ModelConfig:
    ...
    # derived external describability projection (absolute-state + transition driven)
    eta_floor: float = 0.30
    eta_ceiling: float = 0.98
    eta_sigma_weight: float = 0.22
    eta_dw_weight: float = 0.20
    eta_density_weight: float = 0.18
    eta_flow_weight: float = 0.15
    eta_transition_weight: float = 0.25
    eta_transition_sharpness: float = 5.0
    eta_center: float = 0.52

    tag_name: str = "describability_gap_v4"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="describability_gap_v4", help="Tag appended to timestamp folder name.")
...
def derive_external_projection(seg: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    ...
    flow_log = np.log1p(np.clip(np.abs(out["I_int_i"].to_numpy(dtype=float)), 0.0, None))
    ...
    flow_term = sat_unit(flow_log, positive_median(flow_log))
    ...
    raw = (
        ...
        + config.eta_density_weight * density_term
        + config.eta_flow_weight * flow_term
...
def summarize_galaxy(point_df: pd.DataFrame, seg_df: pd.DataFrame) -> Dict[str, float]:
    ...
    return {
        ...
        "eta_transition_mean": float(np.mean(point_df["eta_transition_term_i"].to_numpy(dtype=float))),
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v4.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v4.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v4.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "Baryonic Newton baseline preserved. External describability is derived from absolute structural-state quantities and a transition term. "
            "Unlike v3, v4 replaces min-max ranking with saturating normalization and interpolates segment diagnostics to pointwise radii to reduce staircase artifacts."
        ),
        "derived_projection": {
            "eta_sigma_term_i": "sigma_i / (sigma_i + median_positive_sigma)",
            "eta_dw_term_i": "|Dw_i-D_star| / (|Dw_i-D_star| + median_positive_Dw_dev)",
            "eta_density_term_i": "log(1+rho_i) / (log(1+rho_i) + median_positive_log_density)",
            "eta_flow_term_i": "log(1+|I_int_i|) / (log(1+|I_int_i|) + median_positive_log_flow)",
            "eta_transition_term_i": "local transition of weighted state combo, then saturating normalization",
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v4.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v4.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v4.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v4.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v4.csv", index=False)
    ...
    aggregate = {
        ...
        "mean_eta_transition_mean": float(summary_df["eta_transition_mean"].mean()) if not summary_df.empty else float("nan"),
    }
    (run_dir / "aggregate_metrics_v4.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
