...
"""
SPARC x Kautsch 2006 crossmatch (v3, robust SPARC column detection)

Why this version exists
-----------------------
The prior v2 script failed with:
    Missing required SPARC columns. name=galaxy_id, ra=None, dec=None

This version fixes that by:
1. Using explicit fallback candidates for SPARC RA/DEC columns
   (RAJ2000, DEJ2000, ra_deg, dec_deg, etc.)
2. Normalizing column names before detection
3. Treating J-names as valid identifiers
4. Using coordinate matching as the main path for Kautsch 2006
5. Writing a nearest-neighbor diagnostic CSV
"""
...
def find_first_existing_column(df: pd.DataFrame, candidates: Iterable[str]) -> Optional[str]:
    actual = {str(c): normalize_colname(str(c)) for c in df.columns}
    norm_to_actual = {}
    for col, norm in actual.items():
        norm_to_actual.setdefault(norm, col)

    for cand in candidates:
        cand_norm = normalize_colname(cand)
        if cand_norm in norm_to_actual:
            return norm_to_actual[cand_norm]
    return None
...
def extract_catalog_token(name: object) -> str:
    """
    Extract traditional catalog tokens and also recognize J-names.
    Examples:
      NGC0891 -> NGC891
      UGC07690 -> UGC7690
      J000003.33-104315.8 -> J000003.33-104315.8
    """
    s = normalize_name(name)
    if not s:
        return ""

    # J-name, preserve as identifier
    if re.fullmatch(r"J\d{6}(\.\d+)?[+-]\d{6}(\.\d+)?", s):
        return s

    # Traditional catalog styles
    m = re.match(r"^(NGC|UGC|IC|ESO|PGC|DDO|UGCA|F)([-A-Z0-9]+)$", s)
    if m:
        prefix, rest = m.groups()

        # Separate pure numeric tail from mixed tails
        if rest.isdigit():
            rest = str(int(rest))
...
def pick_sparc_columns(df: pd.DataFrame) -> tuple[str, str, str]:
    # Name column
    ...
    # RA/DEC columns with explicit strong fallbacks
    ra_col = find_first_existing_column(
        df,
        [
            "RAJ2000",
            "_RAJ2000",
    ...
    dec_col = find_first_existing_column(
        df,
        [
            "DEJ2000",
            "_DEJ2000",
    ...
    if not all([name_col, ra_col, dec_col]):
        raise SystemExit(
            f"Missing required SPARC columns. name={name_col}, ra={ra_col}, dec={dec_col}"
        )

    return name_col, ra_col, dec_col


def collect_sparc_aliases(row: pd.Series, alias_cols: list[str]) -> list[str]:
    vals = []
...
def main() -> int:
    ...
    if not KAUTSCH_FILE.exists():
        raise SystemExit(f"Kautsch prepared CSV not found: {KAUTSCH_FILE}")
    if not SPARC_INDEX_FILE.exists():
        raise SystemExit(f"SPARC galaxy index CSV not found: {SPARC_INDEX_FILE}")

    kautsch = pd.read_csv(KAUTSCH_FILE)
    sparc = pd.read_csv(SPARC_INDEX_FILE)

    # Detect Kautsch columns
    ...
    # Detect SPARC columns
    s_name_col, s_ra_col, s_dec_col = pick_sparc_columns(sparc)

    # Likely alias columns on SPARC side
    sparc_alias_cols = []
    for c in ["name_normalized", "s4g_name_normalized", "Name", "filename", "galaxy_id"]:
        actual = find_first_existing_column(sparc, [c])
        if actual and actual not in sparc_alias_cols:
            sparc_alias_cols.append(actual)
    ...
    # Also try SDSS/J-name as identifier if present
    ...
    summary = {
        ...
        "detected_sparc_columns": {
            "name": s_name_col,
            "ra": s_ra_col,
            "dec": s_dec_col,
        },
        "detected_kautsch_columns": {
            "name": k_name_col,
            "ra": k_ra_col,
            "dec": k_dec_col,
        },
