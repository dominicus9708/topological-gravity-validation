...
PROJECT_ROOT = Path(".")
BIZYAEV_FILE = PROJECT_ROOT / "data" / "derived" / "Bizyaev" / "bizyaev_table4_rband_for_crossmatch.csv"
SPARC_INDEX_FILE = PROJECT_ROOT / "data" / "derived" / "crossmatch" / "sparc_diskmass_S4G" / "sparc_galaxy_index_with_coords.csv"
OUTPUT_DIR = PROJECT_ROOT / "data" / "derived" / "crossmatch" / "sparc_bizyaev" / "recomputed_with_coords_2014"
...
def main() -> int:
    ...
    if not BIZYAEV_FILE.exists():
        raise SystemExit(f"Missing Bizyaev file: {BIZYAEV_FILE}")
    if not SPARC_INDEX_FILE.exists():
        raise SystemExit(f"Missing SPARC coordinate index: {SPARC_INDEX_FILE}")

    biz = pd.read_csv(BIZYAEV_FILE)
    sparc = pd.read_csv(SPARC_INDEX_FILE)

    b_name_col = find_first_existing_column(biz, ["preferred_name", "AName", "NED", "Name", "catalog_name_normalized"])
    ...
    if not all([b_name_col, b_ra_col, b_dec_col]):
        raise SystemExit(f"Missing required Bizyaev columns. name={b_name_col}, ra={b_ra_col}, dec={b_dec_col}")
    ...
    all_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_all.csv"
    exact_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_exact.csv"
    coord_review_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_coord_review.csv"
    matched_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_matched.csv"
    unmatched_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_unmatched.csv"
    nearest_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_nearest_neighbors.csv"
    summary_path = OUTPUT_DIR / "sparc_bizyaev2014_with_coords_crossmatch_summary.json"
    ...
    summary = {
        ...
        "median_z0_over_h_matched": (
            float(pd.to_numeric(matched_df.get("z0_over_h"), errors="coerce").median())
            if "z0_over_h" in matched_df.columns and len(matched_df) > 0 else None
        ),
        "median_h_over_z0_matched": (
            float(pd.to_numeric(matched_df.get("h_over_z0"), errors="coerce").median())
            if "h_over_z0" in matched_df.columns and len(matched_df) > 0 else None
        ),
        ...
        "detected_bizyaev_columns": {"name": b_name_col, "ra": b_ra_col, "dec": b_dec_col, "aname": aname_col, "ned": ned_col},
    }
