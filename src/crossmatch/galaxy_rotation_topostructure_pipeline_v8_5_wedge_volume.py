...
def compute_weighted_effective_dimension(
    ...
    mass_bar_msun: float,
    r_ref_kpc: float,
    density_ref_msun_per_kpc3: float,
) -> Tuple[float, float, float, float, float, float, float]:
    """
    Wedge-volume version.

    The azimuthal opening angle is treated as a galaxy-wide constant, so it is
    absorbed into the measure normalization and dropped from the effective
    volume. The remaining 3D segment measure is therefore proportional to

        V_i^eff ~ h_i * (r_right^2 - r_left^2) / 2.

    This keeps the vertical thickness and radial area growth while avoiding an
    unobservable extra angular degree of freedom in the current crossmatched
    rotation-curve inputs.
    """
    r_left = max(float(r_left_kpc), DEFAULT_EPS)
    r_right = max(float(r_right_kpc), r_left + DEFAULT_EPS)
    r_mid = max(float(r_mid_kpc), DEFAULT_EPS)
    h_eff = max(float(h_mid_kpc), DEFAULT_EPS)
    mass_eff = max(float(mass_bar_msun), DEFAULT_EPS)
    r_ref = max(float(r_ref_kpc), DEFAULT_EPS)
    rho_ref = max(float(density_ref_msun_per_kpc3), DEFAULT_EPS)

    wedge_area_eff = max(0.5 * (r_right ** 2 - r_left ** 2), DEFAULT_EPS)
    wedge_volume_eff = max(h_eff * wedge_area_eff, DEFAULT_EPS)
    rho_eff = mass_eff / wedge_volume_eff

    radial_log = max(math.log(1.0 + r_mid / r_ref), 1e-6)
    area_scale = max(math.sqrt(wedge_area_eff), DEFAULT_EPS)

    # Geometric contribution: 2D annular patch -> 3D thickened patch.
    alpha_geom = 2.0 + math.log(1.0 + h_eff / area_scale) / radial_log

    # Density contribution: denser occupation of the 3D wedge means stronger
    # structural filling of the available segment measure.
    alpha_density = 2.0 + math.log(1.0 + rho_eff / rho_ref) / radial_log

    alpha_geom = float(np.clip(alpha_geom, 2.0, 3.5))
    alpha_density = float(np.clip(alpha_density, 2.0, 3.5))

    thickness_ratio = h_eff / (h_eff + area_scale)
    density_ratio = rho_eff / (rho_eff + rho_ref)
    weight_sum = max(thickness_ratio + density_ratio, DEFAULT_EPS)

    dw = (thickness_ratio * alpha_geom + density_ratio * alpha_density) / weight_sum
    dw = float(np.clip(dw, 2.0, 3.5))
    return dw, alpha_geom, alpha_density, thickness_ratio, density_ratio, wedge_volume_eff, rho_eff
...
def build_segments_base(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    wedge_volume_seed = np.clip(
        0.5 * (seg_table["r_right_kpc"].to_numpy(dtype=float) ** 2 - seg_table["r_left_kpc"].to_numpy(dtype=float) ** 2)
        * np.clip(seg_table["h_i_kpc"].to_numpy(dtype=float), DEFAULT_EPS, None),
        DEFAULT_EPS,
        None,
    )
    density_seed = np.clip(seg_table["mass_bar_segment_msun"].to_numpy(dtype=float), DEFAULT_EPS, None) / wedge_volume_seed
    density_ref = float(np.nanmedian(density_seed))
    density_ref = max(density_ref, 1e6)
    seg_table["density_ref_gal_msun_per_kpc3"] = density_ref

    dw_vals: List[float] = []
    alpha_geom_vals: List[float] = []
    alpha_density_vals: List[float] = []
    w_geom_vals: List[float] = []
    w_density_vals: List[float] = []
    wedge_volume_vals: List[float] = []
    rho_vals: List[float] = []
    for _, row in seg_table.iterrows():
        dw, alpha_geom, alpha_density, w_geom, w_density, wedge_volume_eff, rho_eff = compute_weighted_effective_dimension(
            r_left_kpc=float(row["r_left_kpc"]),
            r_right_kpc=float(row["r_right_kpc"]),
            r_mid_kpc=float(row["r_mid_kpc"]),
            h_mid_kpc=float(row["h_i_kpc"]),
            mass_bar_msun=float(row["mass_bar_segment_msun"]),
            r_ref_kpc=r_ref_kpc,
            density_ref_msun_per_kpc3=density_ref,
        )
        dw_vals.append(dw)
        alpha_geom_vals.append(alpha_geom)
        alpha_density_vals.append(alpha_density)
        w_geom_vals.append(w_geom)
        w_density_vals.append(w_density)
        wedge_volume_vals.append(wedge_volume_eff)
        rho_vals.append(rho_eff)

    seg_table["segment_wedge_volume_eff_kpc3"] = wedge_volume_vals
    seg_table["segment_density_eff_msun_per_kpc3"] = rho_vals
    seg_table["alpha_geom_i"] = alpha_geom_vals
    seg_table["alpha_density_i"] = alpha_density_vals
    seg_table["weight_geom_i"] = w_geom_vals
    seg_table["weight_density_i"] = w_density_vals
