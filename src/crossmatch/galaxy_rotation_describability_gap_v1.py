...
class ModelConfig:
    ...
    # plot/output
    tag_name: str = "describability_gap_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="describability_gap_v1", help="Tag appended to timestamp folder name.")
...
def build_segment_table(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    for i in range(len(S)):
        kernel = np.exp(-np.abs(r_mid[i] - r_mid) / max(ell_info, DEFAULT_EPS))
        I_int[i] = float(np.sum(S * kernel))
    seg["I_int_i"] = I_int
    seg["I_ext_i"] = config.eta_obs * seg["I_int_i"]
    seg["DeltaD_i"] = np.abs(seg["I_int_i"] - seg["I_ext_i"]) / np.clip(np.abs(seg["I_int_i"]), DEFAULT_EPS, None)
    return seg
...
def summarize_galaxy(point_df: pd.DataFrame, seg_df: pd.DataFrame) -> Dict[str, float]:
    ...
    return {
        ...
        "eta_obs": float(seg_df["I_ext_i"].iloc[0] / max(seg_df["I_int_i"].iloc[0], DEFAULT_EPS)),
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v1.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v1.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v1.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": "Baryonic Newton baseline preserved. The pipeline estimates internal/external describability-gap indicators rather than adding a direct topological acceleration term.",
    }
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v1.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v1.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v1.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v1.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v1.csv", index=False)
    ...
    aggregate = {
        ...
        "mean_M_gal": float(pd.DataFrame(summary_rows)["M_gal"].mean()) if summary_rows else float("nan"),
        "mean_R_gal": float(pd.DataFrame(summary_rows)["R_gal"].mean()) if summary_rows else float("nan"),
    }
    (run_dir / "aggregate_metrics_v1.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
