...
class ModelConfig:
    ...
    # describability gap from propagation only
    delta_floor: float = 0.0
    delta_ceiling: float = 1.0
    delta_p0_scale: float = 1.0

    tag_name: str = "describability_gap_v6"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Occam-style crossmatch pipeline for describability-gap diagnostics on galaxy rotation data."
    )
    ...
    parser.add_argument("--tag", default="describability_gap_v6", help="Tag appended to timestamp folder name.")
...
def build_segment_table(df: pd.DataFrame, galaxy: str, config: ModelConfig) -> pd.DataFrame:
    ...
    sigma_term = sat_unit(seg["sigma_i"].to_numpy(dtype=float), positive_median(seg["sigma_i"].to_numpy(dtype=float), fallback=0.1))
    seg["propagation_source_i"] = np.power(sigma_term, config.propagation_source_power)
    ...
    for i in range(len(seg)):
        ...
        for j in range(i + 1):
            ...
            attenuation = math.exp(-radial_gap / max(ell_info, DEFAULT_EPS))
            shell_weight = dr[j] / max(r_max, DEFAULT_EPS)
            total += float(seg["propagation_source_i"].iloc[j]) * attenuation * shell_weight
        P_raw[i] = total
...
def plot_one_galaxy(point_df: pd.DataFrame, out_path: Path, dpi: int) -> None:
    ...
    ax.plot(x, d["propagation_term_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="cumulative propagation")
    ax.plot(x, d["DeltaD_i"].to_numpy(dtype=float), marker="o", linewidth=1.2, label="describability gap")
    ax.set_ylabel("Propagation-driven gap")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")
...
def merge_outputs(point_frames: List[pd.DataFrame], seg_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if point_frames:
        pd.concat(point_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_describability_gap_v6.csv", index=False)
    if seg_frames:
        pd.concat(seg_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_describability_gap_v6.csv", index=False)
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(merged_dir / "merged_summary_describability_gap_v6.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": (
            "Occam-style v6: keep only baryonic Newton baseline, Dw, sigma, outward cumulative propagation P, and a saturating propagation-to-gap map. "
            "External describability gap is modeled solely as a function of cumulative outward propagation."
        ),
        "minimal_story": {
            "Dw_i": "effective structural dimension per segment",
            "sigma_i": "local structural mismatch from neighboring segments",
            "Q_i": "propagation source taken as sigma_i only (saturating normalized)",
            "P_i": "outward cumulative attenuated sum of Q_i from inner shells",
...
def main() -> None:
    ...
    for file_path in sorted(input_dir.glob(args.glob)):
        ...
        try:
            ...
            point.to_csv(per_galaxy_dir / f"{galaxy}_pointwise_describability_gap_v6.csv", index=False)
            seg.to_csv(per_galaxy_dir / f"{galaxy}_segment_describability_gap_v6.csv", index=False)
            plot_one_galaxy(point, plots_dir / f"{galaxy}_describability_gap_v6.png", config.dpi)
    ...
    if summary_rows:
        pd.DataFrame(summary_rows).to_csv(run_dir / "describability_gap_summary_v6.csv", index=False)
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "describability_gap_failures_v6.csv", index=False)
