...
Topological pipeline for common5 final input (v8)
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    note_text = (
        ...
        f"Inner = 0 to {zone.inner_outer_ratio:.1f}R. "
        f"Shell = {zone.shell_inner_ratio:.1f}R to {zone.shell_outer_ratio:.1f}R. "
        f"Background = {zone.bg_inner_ratio:.1f}R to {zone.bg_outer_ratio:.1f}R."
    )
    fig.text(
        0.10,
        0.055,
    ...
    legend_lines = [
        ...
        (f"White gaps are the unfilled intervals between these zones", 9.2, "left"),
        (f"Reference radius: R = {zone.radius_arcsec:.1f} arcsec", 9.2, "left"),
    ]

    max_chars = max(len(txt) for txt, _, _ in legend_lines)
    legend_width = min(max(0.31, 0.11 + max_chars * 0.010), 0.48)
    legend_height = (len(legend_lines) - 1) * 0.0025 + 0.14
    legend_right = 0.97
    ...
    # ratio-faithful zone fills from per-target boundaries
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v8_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v8.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v8)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v8 pipeline reads the distance-enriched final input and radial profiles.",
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v8.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v8)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v8.csv")
