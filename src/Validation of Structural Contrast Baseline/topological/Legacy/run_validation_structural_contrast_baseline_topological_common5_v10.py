...
Topological pipeline for common5 final input (v10)
...
Input
-----
data/derived/Validation of Structural Contrast Baseline/input/topological/wise_hii_common5/wise_hii_common5_topological_final_input_distance_enriched_v1.csv
...
INPUT_FILE = (
    ...
    / "topological"
    / "wise_hii_common5"
    / "wise_hii_common5_topological_final_input_distance_enriched_v1.csv"
)
...
class TargetZones:
    ...
    shell_inner_ratio: float
    shell_outer_ratio: float
    bg_inner_ratio: float
    bg_outer_ratio: float
...
def build_standard_zones(df: pd.DataFrame) -> List[TargetZones]:
    def get_ratio(row: pd.Series, col: str, default: float) -> float:
    ...
    for _, row in df.iterrows():
        ...
        # per-target ratio support:
        # if these columns exist in the input, they override the old fixed defaults
        inner_outer_ratio = get_ratio(row, "inner_outer_ratio", 0.50)
        shell_inner_ratio = get_ratio(row, "shell_inner_ratio", 0.60)
        shell_outer_ratio = get_ratio(row, "shell_outer_ratio", 1.00)
        bg_inner_ratio = get_ratio(row, "bg_inner_ratio", 1.20)
        bg_outer_ratio = get_ratio(row, "bg_outer_ratio", 1.80)
        ...
        zones.append(
            TargetZones(
                ...
                inner_outer_arcsec=round(max(inner_outer_ratio * r, 1.0), 3),
                shell_inner_arcsec=round(shell_inner_ratio * r, 3),
                shell_outer_arcsec=round(shell_outer_ratio * r, 3),
                bg_inner_arcsec=round(bg_inner_ratio * r, 3),
                bg_outer_arcsec=round(bg_outer_ratio * r, 3),
                inner_outer_ratio=inner_outer_ratio,
                shell_inner_ratio=shell_inner_ratio,
                shell_outer_ratio=shell_outer_ratio,
                bg_inner_ratio=bg_inner_ratio,
                bg_outer_ratio=bg_outer_ratio,
            )
        )
    return zones
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path, global_rmax_arcsec: float) -> None:
    # Common outer plotting scale across all targets.
    rmax = float(global_rmax_arcsec)
    center_circle_r = max(0.010 * rmax, 0.9)

    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_axes([0.06, 0.16, 0.72, 0.72])
    ...
    # Keep note short and based on actual arcsec boundaries used in the line plots
    note_text = (
        f"Note. Common outer scale across all targets. "
        f"Actual arcsec boundaries used here are the same boundaries used in the line plots."
    )
    fig.text(
        0.08,
        0.055,
    ...
    legend_lines = [
        ("Legend", 12, "center"),
        ("Yellow dot: center marker", 9.7, "left"),
        (f"Inner fill: 0.0\" to {zone.inner_outer_arcsec:.1f}\"", 9.2, "left"),
        (f"Shell fill: {zone.shell_inner_arcsec:.1f}\" to {zone.shell_outer_arcsec:.1f}\"", 9.2, "left"),
        (f"Background fill: {zone.bg_inner_arcsec:.1f}\" to {zone.bg_outer_arcsec:.1f}\"", 9.2, "left"),
        (f"Reference radius R = {zone.radius_arcsec:.1f}\"", 9.2, "left"),
        (f"Common plotting outer radius = {global_rmax_arcsec:.1f}\"", 9.2, "left"),
    ]

    max_chars = max(len(txt) for txt, _, _ in legend_lines)
    legend_width = min(max(0.34, 0.11 + max_chars * 0.010), 0.52)
    legend_height = (len(legend_lines) - 1) * 0.0025 + 0.15
    legend_right = 0.97
    ...
    for i, (txt, fs, align) in enumerate(legend_lines):
        ...
        y -= 0.12 if i == 0 else 0.108

    ax.set_aspect("equal")
    ax.set_xlim(-rmax * 1.08, rmax * 1.08)
    ax.set_ylim(-rmax * 1.08, rmax * 1.08)

    ax.plot([-rmax, rmax], [0, 0], linewidth=1.0, color="#2C7FB8")
    ax.plot([0, 0], [-rmax, rmax], linewidth=1.0, color="#D95F0E")

    # Actual arcsec widths on the common scale
    ...
    # Exact boundaries
    for rad, lw in [
        (zone.inner_outer_arcsec, 1.2),
        (zone.shell_outer_arcsec, 1.4),
        (zone.bg_outer_arcsec, 1.2),
    ]:
    ...
    for rad in [zone.shell_inner_arcsec, zone.bg_inner_arcsec]:
        ax.add_patch(Circle((0, 0), rad, fill=False, linestyle="--", linewidth=1.25, edgecolor="black"))

    ax.add_patch(Circle((0, 0), center_circle_r, facecolor="#FFD92F", edgecolor="black", linewidth=1.0))

    # Labels at actual annulus midpoints
    ax.text(0, 0.55 * zone.inner_outer_arcsec, "Inner", ha="center", va="center", fontsize=16)
    ax.text(0, 0.5 * (zone.shell_inner_arcsec + zone.shell_outer_arcsec), "Shell", ha="center", va="center", fontsize=16)
    ax.text(0, 0.5 * (zone.bg_inner_arcsec + zone.bg_outer_arcsec), "Background", ha="center", va="center", fontsize=16)

    # Tick labels in actual arcsec, same boundaries as line plots
    tick_len_major = 8
    major_ticks = [
        (f'{zone.inner_outer_arcsec:.1f}"', zone.inner_outer_arcsec),
        (f'{zone.shell_outer_arcsec:.1f}"', zone.shell_outer_arcsec),
        (f'{zone.bg_outer_arcsec:.1f}"', zone.bg_outer_arcsec),
    ]
    for lab, rad in major_ticks:
        ax.plot([rad, rad], [-tick_len_major, tick_len_major], color="black", linewidth=1.2)
        ax.annotate(
            lab,
            xy=(rad, 0),
            xytext=(0, 12),
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v10_summary.txt").write_text(
        "\n".join(lines), encoding="utf-8"
    )


def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path, global_rmax_arcsec: float) -> dict:
    wise_name = str(row["wise_name"])
    ...
    save_circular_plot_for_target(row, zone, target_dir, global_rmax_arcsec)
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v10.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v10)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v10 pipeline reads the distance-enriched final input and radial profiles.",
...
def main() -> int:
    ...
    global_rmax_arcsec = max(z.bg_outer_arcsec for z in zones)
    ...
    for _, row in df.iterrows():
        ...
        results.append(compute_target_result(row, zone, project_root, target_dir, global_rmax_arcsec))

    summary_df = pd.DataFrame(results)
    summary_df.to_csv(out_dir / "topological_summary_common5_v10.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v10)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v10.csv")
