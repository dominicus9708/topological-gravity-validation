...
Topological pipeline for common5 final input (v3)
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v3_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v3.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v3)",
        "=" * 60,
        "",
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v3.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v3)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v3.csv")
