...
Topological pipeline for common5 final input (v9)
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path, global_rmax_arcsec: float) -> None:
    # Use a single common outer plotting scale across all targets in the run.
    rmax = float(global_rmax_arcsec)
    center_circle_r = max(0.010 * rmax, 0.9)

    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_axes([0.06, 0.15, 0.72, 0.74])
    ...
    note_text = (
        ...
        f"All circular plots in this run use the same outer plotting scale so that inter-target width differences remain visible."
    )
    fig.text(
        0.08,
        0.055,
        wrap_note_by_periods(note_text, width=90),
        ha="left",
        va="bottom",
        fontsize=9.5,
    )
    ...
    legend_lines = [
        ...
        ("Common outer scale across all targets in this run", 9.2, "left"),
        (f"Inner fill: 0.0R to {zone.inner_outer_ratio:.1f}R", 9.2, "left"),
        (f"Shell fill: {zone.shell_inner_ratio:.1f}R to {zone.shell_outer_ratio:.1f}R", 9.2, "left"),
        (f"Background fill: {zone.bg_inner_ratio:.1f}R to {zone.bg_outer_ratio:.1f}R", 9.2, "left"),
        (f"Reference radius: R = {zone.radius_arcsec:.1f} arcsec", 9.2, "left"),
    ]

    max_chars = max(len(txt) for txt, _, _ in legend_lines)
    legend_width = min(max(0.33, 0.11 + max_chars * 0.010), 0.50)
    ...
    for i, (txt, fs, align) in enumerate(legend_lines):
        legend_ax.text(0.5 if align == "center" else 0.05, y, txt, ha=align, va="center", fontsize=fs)
    ...
    # Draw actual arcsec radii on the common scale
    ...
    tick_len_major = 8
    major_ticks = [
        (f"{zone.inner_outer_ratio:.1f}R", zone.inner_outer_arcsec),
        (f"{zone.shell_outer_ratio:.1f}R", zone.shell_outer_arcsec),
        (f"{zone.bg_outer_ratio:.1f}R", zone.bg_outer_arcsec),
    ]
    for lab, rad in major_ticks:
        ax.plot([rad, rad], [-tick_len_major, tick_len_major], color="black", linewidth=1.2)
        ax.annotate(
            make_radius_label(lab, rad),
            xy=(rad, 0),
            xytext=(0, 12),
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v9_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path, global_rmax_arcsec: float) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v9.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v9)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v9 pipeline reads the distance-enriched final input and radial profiles.",
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v9.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v9)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v9.csv")
