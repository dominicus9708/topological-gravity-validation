...
Topological pipeline for common5 final input (v5)
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    note_text = (
        ...
        f"The center marker is intentionally tiny so that radial spacing remains visible. "
        f"This figure is a ratio-aware zone map: the circular boundaries follow the same exact radii used in the linear plots "
        f"(0.5R, 0.6R, 1.0R, 1.2R, 1.8R). The 0.5R and 1.0R references are shown as tick marks on the horizontal axis, "
        f"not as extra circular labels."
    )
    fig.text(
        0.10,
        0.05,
        wrap_note_by_periods(note_text, width=100),
        ha="left",
        va="bottom",
        fontsize=10,
    )
    ...
    legend_lines = [
        ...
        (f"Inner baseline zone: 0.0R to 0.5R (0.0\" to {zone.inner_outer_arcsec:.1f}\")", 9.2, "left"),
        (f"Shell structure zone: 0.6R to 1.0R ({zone.shell_inner_arcsec:.1f}\" to {zone.shell_outer_arcsec:.1f}\")", 9.2, "left"),
        (f"Background baseline zone: 1.2R to 1.8R ({zone.bg_inner_arcsec:.1f}\" to {zone.bg_outer_arcsec:.1f}\")", 9.2, "left"),
        (f"Reference radius: R = {zone.radius_arcsec:.1f} arcsec", 9.2, "left"),
    ]

    max_chars = max(len(txt) for txt, _, _ in legend_lines)
    legend_width = min(max(0.29, 0.11 + max_chars * 0.010), 0.44)
    ...
    # Horizontal-axis tick marks for key radii
    tick_len_major = 8
    tick_len_minor = 5
    major_ticks = [("0.5R", zone.inner_outer_arcsec), ("1.0R", zone.shell_outer_arcsec), ("1.8R", zone.bg_outer_arcsec)]
    minor_ticks = [("0.6R", zone.shell_inner_arcsec), ("1.2R", zone.bg_inner_arcsec)]

    for lab, rad in major_ticks:
        ax.plot([rad, rad], [-tick_len_major, tick_len_major], color="black", linewidth=1.2)
        ax.text(rad, tick_len_major + 3, make_radius_label(lab, rad), fontsize=10.2, ha="center", va="bottom")

    for lab, rad in minor_ticks:
        ax.plot([rad, rad], [-tick_len_minor, tick_len_minor], color="black", linewidth=1.0, linestyle="--")
        ax.text(rad, -(tick_len_minor + 5), make_radius_label(lab, rad), fontsize=10.0, ha="center", va="top")
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v5_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v5.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v5)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v5 pipeline reads the distance-enriched final input and radial profiles.",
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v5.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v5)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v5.csv")
