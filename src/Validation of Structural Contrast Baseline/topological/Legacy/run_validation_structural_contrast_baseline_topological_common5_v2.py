...
Topological pipeline for common5 final input (v2)
...
def derive_mass_proxy(row: pd.Series) -> tuple[Optional[float], str]:
    ...
    if pd.notna(proxy_value) and proxy_kind in {"radio_continuum", "log_nly"}:
        return float(proxy_value), f"proxy_value_from_{proxy_kind}"
...
def derive_physical_radius_scale_pc(row: pd.Series) -> tuple[Optional[float], str]:
    # Prefer explicit cutout physical sizes if ever available later; currently use distance+angular radius
...
def classify_v2(status: str, sigma_mass_volume: Optional[float]) -> str:
...
def save_mass_bridge_plot(wise_name: str, D_shell: Optional[float], D_bg: Optional[float], shell_mv_density: Optional[float], bg_mv_density: Optional[float], out_dir: Path) -> None:
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v2_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v2.csv",
            index=False,
            encoding="utf-8-sig",
        )

    flag = classify_v2(status, sigma_mass_volume)
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v2)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v2 pipeline reads fixed final input and radial profiles.",
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v2.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v2)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v2.csv")
