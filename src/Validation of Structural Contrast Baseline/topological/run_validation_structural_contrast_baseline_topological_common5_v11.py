...
Topological pipeline for common5 final input (v11)
...
- Added:
  1) Dw_obs zone bar plot
  2) previous alpha_obs and L(<r) plots retained
  3) mass-volume bridge plot retained
  4) circular schematic removed in v11
"""
...
def save_summary_txt(
    ...
    (out_dir / f"{sanitize_wise_name(wise_name)}_topological_common5_v11_summary.txt").write_text(
...
def compute_target_result(row: pd.Series, zone: TargetZones, project_root: Path, target_dir: Path) -> dict:
    ...
    if profile_path is not None:
        ...
        profile_struct.to_csv(
            target_dir / f"{sanitize_wise_name(wise_name)}_processed_structure_profile_common5_v11.csv",
            index=False,
            encoding="utf-8-sig",
        )
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Topological pipeline manifest (common5 v11)",
        "=" * 60,
        "",
        ...
        "meaning",
        "-" * 20,
        "This common5 v11 pipeline reads the distance-enriched final input and radial profiles.",
        ...
        "Added graphs: alpha_obs, L(<r), Dw_obs zone bar, and mass-volume bridge.",
        "Circular schematic was removed in v11.",
    ]
...
def main() -> int:
    ...
    summary_df.to_csv(out_dir / "topological_summary_common5_v11.csv", index=False, encoding="utf-8-sig")

    write_manifest(project_root, out_dir, df)
    write_run_summary(out_dir, summary_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - topological pipeline (common5 v11)")
    print("=" * 72)
    ...
    print("")
    print("[OK] Created:")
    print(out_dir / "topological_summary_common5_v11.csv")
