...
Standard pipeline (circular schematic, revised v2)

Changes
-------
1. Korean font handling improved for Windows (Malgun Gothic preferred).
2. R meaning is explained directly in the figure.
3. Circular regions are visually separated by filled annuli.
4. Central schematic circle is filled yellow for clarity.
5. Labels are more explicit and include arcsec values.
6. The circular plot adapts its annotation placement to each target radius.
"""
...
def set_korean_font() -> None:
    # Best-effort Windows Korean font setting
    try:
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    ax = fig.add_axes([0.06, 0.07, 0.72, 0.82])
    legend_ax = fig.add_axes([0.80, 0.68, 0.18, 0.24])
    title_ax = fig.add_axes([0.22, 0.91, 0.42, 0.075])
    note_ax = fig.add_axes([0.14, 0.01, 0.62, 0.05])

    # Title box
    title_ax.set_axis_off()
    title_ax.add_patch(Rectangle((0, 0), 1, 1, fill=False, linewidth=1.2))
    title_ax.text(0.5, 0.52, f"{target['wise_name']} 표준 기준 구역 도식",
                  ha="center", va="center", fontsize=16)

    # Note box: explain R in direct terms
    note_ax.set_axis_off()
    note_ax.add_patch(Rectangle((0, 0), 1, 1, fill=False, linewidth=0.8))
    note_ax.text(
        0.02, 0.5,
        f"주: R은 카탈로그 기준 반지름이며, 이 대상에서는 R = {zone.radius_arcsec:.1f} arcsec 입니다. "
        "중심의 작은 원은 도식적 중심 표시이며 실제 축척이 아닙니다.",
    ...
    legend_lines = [
        "범례",
        "노란 중심 원: 중심 기준점",
        f"내부 기준 구역: 0 ~ 0.5R ({zone.inner_outer_arcsec:.1f}\")",
        f"쉘 구조 구역: 0.6R ~ 1.0R ({zone.shell_inner_arcsec:.1f}\" ~ {zone.shell_outer_arcsec:.1f}\")",
        f"배경 기준 구역: 1.2R ~ 1.8R ({zone.bg_inner_arcsec:.1f}\" ~ {zone.bg_outer_arcsec:.1f}\")",
        f"기준 반경: R = {zone.radius_arcsec:.1f}\"",
    ]
    y = 0.88
    for i, txt in enumerate(legend_lines):
        fs = 12 if i == 0 else 9.8
        legend_ax.text(
            0.5 if i == 0 else 0.05,
            y,
            txt,
    ...
    # Filled zones
    # background annulus
    ax.add_patch(Circle((0, 0), zone.bg_outer_arcsec, facecolor="#C6DBEF", edgecolor="black", linewidth=1.0, alpha=0.65))
    ax.add_patch(Circle((0, 0), zone.bg_inner_arcsec, facecolor="white", edgecolor="none"))
    # shell annulus
    ax.add_patch(Circle((0, 0), zone.shell_outer_arcsec, facecolor="#9ECAE1", edgecolor="black", linewidth=1.0, alpha=0.85))
    ax.add_patch(Circle((0, 0), zone.shell_inner_arcsec, facecolor="white", edgecolor="none"))
    # inner disk
    ax.add_patch(Circle((0, 0), zone.inner_outer_arcsec, facecolor="#FFF7BC", edgecolor="black", linewidth=1.0, alpha=0.95))

    # Catalog radius dashed circle
    ax.add_patch(Circle((0, 0), zone.radius_arcsec, fill=False, linestyle="--", linewidth=1.6, edgecolor="black"))

    # Schematic central circle
    ax.add_patch(Circle((0, 0), center_circle_r, facecolor="#FFD92F", edgecolor="black", linewidth=1.0))

    # Region labels
    ax.text(0, zone.inner_outer_arcsec * 0.52, "내부", ha="center", va="center", fontsize=15)
    ax.text(0, (zone.shell_inner_arcsec + zone.shell_outer_arcsec) / 2, "쉘", ha="center", va="center", fontsize=15)
    ax.text(0, (zone.bg_inner_arcsec + zone.bg_outer_arcsec) / 2, "배경", ha="center", va="center", fontsize=15)

    # R markers with both ratio and arcsec
    label_x_offset = 7
    ax.text(zone.inner_outer_arcsec + label_x_offset, 5, f"0.5R\n({zone.inner_outer_arcsec:.1f}\")", fontsize=10, ha="left", va="center")
    ax.text(zone.shell_outer_arcsec + label_x_offset, 5, f"1.0R\n({zone.shell_outer_arcsec:.1f}\")", fontsize=10, ha="left", va="center")
    ax.text(zone.bg_outer_arcsec + label_x_offset, 5, f"1.8R\n({zone.bg_outer_arcsec:.1f}\")", fontsize=10, ha="left", va="center")
...
def main() -> int:
    ...
    print("=" * 72)
    print("Validation of Structural Contrast Baseline - standard pipeline (circular v2)")
    print("=" * 72)
