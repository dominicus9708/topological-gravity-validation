...
Standard pipeline for common5 final input (circular schematic)
...
def normalize_input(df: pd.DataFrame) -> pd.DataFrame:
    ...
    required = [
        ...
        "ra", "dec", "radius_arcsec", "baseline_ready_flag"
    ]
    ...
    not_ready = df.loc[~df["baseline_ready_flag"].astype(str).str.lower().isin(["true", "yes", "1"]), "wise_name"].tolist()
    if not_ready:
        raise ValueError("Rows not baseline_ready_flag=True/yes: " + ", ".join(not_ready))
...
def build_baseline_comparison_table(df: pd.DataFrame, zones_df: pd.DataFrame) -> pd.DataFrame:
    ...
    keep_cols = [
        "wise_name", "catalog_class", "input_group", "priority_rank", "region_bucket", "score_total",
        "ra", "dec", "radius_arcsec", "zone_radius_arcsec",
        "inner_inner_arcsec", "inner_outer_arcsec", "shell_inner_arcsec", "shell_outer_arcsec",
        "bg_inner_arcsec", "bg_outer_arcsec", "fits_source_service", "fits_band", "fits_url",
        "fits_downloadable", "fits_local_path", "fits_local_verified", "fits_image_plane_verified",
        "fits_check_status", "fits_notes", "baseline_geometry_status", "baseline_ready_for_profile_extraction",
    ]
    for c in keep_cols:
...
def save_linear_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    ax.set_title(f"{target['wise_name']} common5 standard baseline zones")
    ax.set_ylim(0, 1)
    ax.set_yticks([])
    ax.legend()
    fig.tight_layout()
...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    fig.text(0.5, 0.95, f"{target['wise_name']} Standard Baseline Zone Schematic", ha="center", va="center", fontsize=18)
    ...
    for rad, lw in [(zone.inner_outer_arcsec, 1.1), (zone.shell_outer_arcsec, 1.4), (zone.bg_outer_arcsec, 1.1)]:
    ...
    ax.text(zone.inner_outer_arcsec, 10, make_radius_label("0.5R", zone.inner_outer_arcsec), fontsize=10.5, ha="center", va="bottom")
    ax.text(zone.shell_outer_arcsec, 10, make_radius_label("1.0R", zone.shell_outer_arcsec), fontsize=10.5, ha="center", va="bottom")
    ax.text(zone.bg_outer_arcsec, 10, make_radius_label("1.8R", zone.bg_outer_arcsec), fontsize=10.5, ha="center", va="bottom")
...
def save_per_target_summary(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    lines = [
        ...
        f"region_bucket: {target.get('region_bucket', '')}",
        f"score_total: {target.get('score_total', '')}",
        f"ra: {target.get('ra', '')}",
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
        "Standard common5 pipeline manifest",
        "=" * 60,
        "",
        ...
        "",
        "selected common5 targets",
        "-" * 20,
    ]
    for _, row in df.iterrows():
        lines.append(f"- {row['wise_name']} | rank={row.get('priority_rank', '')} | region={row.get('hii_region_name', '')} | radius_arcsec={row.get('radius_arcsec', '')}")
...
def write_summary_csv(out_dir: Path, df: pd.DataFrame, zones_df: pd.DataFrame) -> None:
    rows = [
        ...
        {"metric": "baseline_ready_count", "value": int(df["baseline_ready_flag"].astype(str).str.lower().isin(["true","yes","1"]).sum())},
...
def main() -> int:
    ...
    print("=" * 72)
    print("Validation of Structural Contrast Baseline - standard pipeline common5")
    print("=" * 72)
    print(f"Project root: {project_root}")
    print(f"Input file  : {project_root / INPUT_FILE}")
    print(f"Output dir  : {out_dir}")
    print("[OK] Created common5 standard outputs")
    return 0
