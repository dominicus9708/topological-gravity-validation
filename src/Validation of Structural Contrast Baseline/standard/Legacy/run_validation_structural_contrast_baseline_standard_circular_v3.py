...
def save_circular_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    # Legend box: right edge aligned to figure margin
    legend_width = 0.24
    legend_right = 0.97
    legend_left = legend_right - legend_width
    legend_ax = fig.add_axes([legend_left, 0.64, legend_width, 0.27])

    # Plain title without box
    fig.text(
        0.5, 0.95,
    ...
    # Bottom note without box, with extra bottom margin
    note_text = (
    ...
    label_x_offset = 8
    ax.text(zone.inner_outer_arcsec + label_x_offset, 7, f"0.5R
({zone.inner_outer_arcsec:.1f}\")", fontsize=10.5, ha="left", va="center")
    ax.text(zone.shell_outer_arcsec + label_x_offset, 7, f"1.0R
({zone.shell_outer_arcsec:.1f}\")", fontsize=10.5, ha="left", va="center")
    ax.text(zone.bg_outer_arcsec + label_x_offset, 7, f"1.8R
({zone.bg_outer_arcsec:.1f}\")", fontsize=10.5, ha="left", va="center")
