...
Standard pipeline (baseline structure stage) for WISE H II first-pass targets
...
It keeps the same upper-folder logic as the skeleton stage, but writes to:

    results/Validation of Structural Contrast Baseline/output/standard/YYYYMMDD_HHMMSS/

Current role of this standard stage
-----------------------------------
Because the project is still at the foundational validation stage, this script
does not yet perform science-grade image photometry or archive cutout extraction.

Instead, it performs the "standard baseline structure stage":
1. Read the fixed final input.
2. Construct deterministic radial zones from the selected catalog radius.
3. Produce a standard baseline comparison table:
   - inner region
   - shell region
   - background region
4. Produce per-target baseline guide plots suitable for the next step.

This is a legitimate standard-stage bridge:
- skeleton confirmed path/input stability
- standard now fixes the baseline comparison geometry
- later topological stage can add sigma-related structure on top

Recommended execution
---------------------
From project root:

python "src\\Validation of Structural Contrast Baseline\\standard\\run_validation_structural_contrast_baseline_standard.py"

Optional project-root argument:

python "src\\Validation of Structural Contrast Baseline\\standard\\run_validation_structural_contrast_baseline_standard.py" "C:\\Users\\mincu\\Desktop\\topological_gravity_project"
"""
...
def build_standard_zones(df: pd.DataFrame) -> List[TargetZones]:
    ...
    for _, row in df.iterrows():
        ...
        # Standard-stage baseline geometry
        inner_i = 0.00
        inner_o = max(0.50 * r, 1.0)
        ...
        zones.append(
            TargetZones(
                ...
                radius_arcsec=r,
                inner_inner_arcsec=round(inner_i, 3),
                inner_outer_arcsec=round(inner_o, 3),
                shell_inner_arcsec=round(shell_i, 3),
                shell_outer_arcsec=round(shell_o, 3),
                bg_inner_arcsec=round(bg_i, 3),
                bg_outer_arcsec=round(bg_o, 3),
            )
        )
    return zones
...
def build_baseline_comparison_table(df: pd.DataFrame, zones_df: pd.DataFrame) -> pd.DataFrame:
    merged = df.merge(zones_df, on="wise_name", how="left")
...
def save_plot_for_target(target: pd.Series, zone: TargetZones, out_dir: Path) -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(target_dir / f"{name}_standard_baseline_zones.png", dpi=160)
    plt.close(fig)
...
def write_manifest(project_root: Path, out_dir: Path, df: pd.DataFrame, zones_df: pd.DataFrame) -> None:
    lines = [
        "Validation of Structural Contrast Baseline",
...
def main() -> int:
    ...
    for _, row in df.iterrows():
        ...
        save_plot_for_target(row, zone, out_dir)

    write_manifest(project_root, out_dir, df, zones_df)
    write_summary_csv(out_dir, df, zones_df)

    print("=" * 72)
    print("Validation of Structural Contrast Baseline - standard pipeline")
    print("=" * 72)
