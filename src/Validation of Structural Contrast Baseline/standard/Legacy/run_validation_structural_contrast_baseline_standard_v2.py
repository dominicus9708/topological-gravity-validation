...
Revised version: fixes radius_arcsec propagation in comparison table.
"""
...
def build_baseline_comparison_table(df: pd.DataFrame, zones_df: pd.DataFrame) -> pd.DataFrame:
    # Avoid ambiguous duplicate radius_arcsec columns by renaming the zone radius first.
    zones_tmp = zones_df.rename(columns={"radius_arcsec": "zone_radius_arcsec"}).copy()
    merged = df.merge(zones_tmp, on="wise_name", how="left")

    # Explicitly preserve original input radius and also expose the zone radius used.
    if "radius_arcsec" not in merged.columns:
        merged["radius_arcsec"] = ""
    merged["zone_radius_arcsec"] = merged["zone_radius_arcsec"].astype(str)
...
def main() -> int:
    ...
    print("=" * 72)
    print("Validation of Structural Contrast Baseline - standard pipeline (revised)")
    print("=" * 72)
