...
# V8.1 concept
#
# Keep the user's V8 structural design as much as possible, but repair the
# dimensional weakness of the structural mass term.
#
# User core:
#   Dw[i]          = 2 + log(1 + h[i]/r[i]) / log(r[i]/0.1)
#   delta_sigma[i] = |Dw[i] - Dw[i-1]|
#   beta[i]        = exp(-delta_sigma[i]/L)
#
# Repair:
#   rho_struct_i is still based on Dw^2 * beta, but structural mass is not the
#   raw cumulative sum of a dimensionless quantity anymore. Instead we form a
#   shell structural mass by multiplying by a shell geometry factor and a
#   structural mass scale:
#
#   DeltaM_struct_i = C_struct * rho_struct_i * (2*pi*r_i*dr_i*h_i)
#   M_struct(<r_i)  = cumulative sum DeltaM_struct_i
#
# This gives the structural gravity term an actual mass dimension compatible
# with G in kpc (km/s)^2 / Msun.
# -----------------------------------------------------------------------------
...
class ModelConfig:
    ...
    dpi: int = 160
    c_struct_msun_per_kpc3: float = 1.0e8
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v8.1 with structural shell mass scaling."
    )
    ...
    parser.add_argument("--tag", default="topostructure_v8_1", help="Optional tag to append after timestamp folder name.")
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, config: ModelConfig):
    ...
    for idx in range(len(edges) - 1):
        ...
        shell_geom = max(2.0 * math.pi * r_mid * dr * h_mid, DEFAULT_EPS)
    ...
    # Structural shell mass with dimensional scaling
    seg_table["rho_struct_proxy_i"] = np.square(seg_table["Dw_i"]) * seg_table["beta_i"]
    seg_table["delta_m_struct_msun"] = config.c_struct_msun_per_kpc3 * seg_table["rho_struct_proxy_i"] * seg_table["shell_geom_kpc3"]
    seg_table["M_struct_i_msun"] = np.cumsum(seg_table["delta_m_struct_msun"])
    seg_table["g_topo_km2s2_per_kpc"] = G_KPC_KMS2_PER_MSUN * seg_table["M_struct_i_msun"] / np.clip(np.square(seg_table["r_mid_kpc"]), DEFAULT_EPS, None)
    ...
    for _, seg in seg_table.iterrows():
        ...
        local["rho_struct_proxy_i"] = float(seg["rho_struct_proxy_i"])
        local["delta_m_struct_msun"] = float(seg["delta_m_struct_msun"])
        local["M_struct_i_msun"] = float(seg["M_struct_i_msun"])
        local["g_topo_km2s2_per_kpc"] = np.clip(G_KPC_KMS2_PER_MSUN * float(seg["M_struct_i_msun"]) / np.clip(np.square(local["r_kpc"]), DEFAULT_EPS, None), 0.0, None)
    ...
    metrics = {
        ...
        "mean_delta_m_struct_msun": float(seg_table["delta_m_struct_msun"].mean()),
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    fig.suptitle(f"{galaxy} Topostructure V8.1 Diagnostics", fontsize=14)
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        pd.concat(pointwise_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_topostructure_v8_1.csv", index=False)
    if segment_frames:
        pd.concat(segment_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_topostructure_v8_1.csv", index=False)
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(merged_dir / "merged_summary_topostructure_v8_1.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": "V8.1 keeps the user's Dw and beta test model, but uses shell geometry and a structural mass scale to form DeltaM_struct with physical mass dimension.",
    }
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure_v8_1.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure_v8_1.csv"
            plot_path = plots_dir / f"{slug}_topostructure_v8_1_diagnostics.png"
    ...
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(run_dir / "topostructure_summary_v8_1.csv", index=False)
        aggregate = {
            ...
            "mean_delta_m_struct_msun": float(summary_df["mean_delta_m_struct_msun"].mean()),
            "mean_v_struct_kms": float(summary_df["mean_v_struct_kms"].mean()),
        }
        (run_dir / "aggregate_metrics_v8_1.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures_v8_1.csv", index=False)
