...
#
# Follow-up to V8.2:
#   - V8.1 used Dw^2 * beta as the informational source and was too strong.
#   - V8.2 used sigma^2 * beta and was too weak.
#
# V8.3 therefore tests intermediate informational-source prescriptions:
#   1) linear sigma source      : source = |sigma| * beta
#   2) hybrid source (default)  : source = (a*|sigma| + b*|Dw-2|) * beta
#
# This preserves the informational-topological gravity interpretation while
# placing the source between "structure itself" and "overly suppressed
# perturbation-only" extremes.
# -----------------------------------------------------------------------------
...
class ModelConfig:
    ...
    # information-source -> acceleration conversion
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v8.3 using linear-sigma and hybrid informational source modes."
    )
    ...
    parser.add_argument("--tag", default="topostructure_v8_3", help="Optional tag to append after timestamp folder name.")
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, config: ModelConfig):
    ...
    sigma_abs = np.abs(seg_table["sigma_i"])
    dw_pert = np.abs(seg_table["Dw_i"] - 2.0)
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    fig.suptitle(f"{galaxy} Topostructure V8.3 Diagnostics", fontsize=14)
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        pd.concat(pointwise_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_topostructure_v8_3.csv", index=False)
    if segment_frames:
        pd.concat(segment_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_topostructure_v8_3.csv", index=False)
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(merged_dir / "merged_summary_topostructure_v8_3.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "model_note": "V8.3 uses intermediate informational source prescriptions between V8.1 and V8.2; default source=(|sigma|+|Dw-2|)*beta.",
    }
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure_v8_3.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure_v8_3.csv"
            plot_path = plots_dir / f"{slug}_topostructure_v8_3_diagnostics.png"
    ...
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(run_dir / "topostructure_summary_v8_3.csv", index=False)
        aggregate = {
        ...
        (run_dir / "aggregate_metrics_v8_3.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures_v8_3.csv", index=False)
