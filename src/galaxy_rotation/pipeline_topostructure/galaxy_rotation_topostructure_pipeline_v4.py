...
# V4 idea
#
# Internal baryonic dynamics and external observability/describability are not
# assumed to be identical. The pipeline therefore inserts a describability
# transform layer between the internal baryonic proxy and the externally
# describable velocity.
#
#   v_bar_internal  ->  v_desc  ->  compare with v_obs
#
# Segment-level structure quantities:
#   D_i      : effective structural dimension proxy
#   sigma_i  : deviation from galaxy-level mean structural state
#   beta_i   : coupling strength
#   omega_i  : structural visibility / describability weight
#   tau_i    : signed describability transform
#
# Final prescription:
#   tau_i   = beta_i * tanh(lambda_sigma * sigma_i) * omega_i * psi_i
#   v_desc  = v_bar_internal * sqrt(clip(1 + tau_i, min_inside_sqrt, None))
# -----------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v4 with describability transform layer."
    )
    ...
    parser.add_argument("--tag", default="topostructure_v4", help="Optional tag to append after timestamp folder name.")
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, thickness: Optional[GalaxyThickness], config: ModelConfig):
    ...
    seg_table["psi_i"] = [psi_function(x, config) for x in seg_table["r_mid_kpc"]]
    ...
    seg_table["omega_i"] = (
        np.power(density_ratio, config.omega_density_power)
        * np.power(thickness_ratio, config.omega_thickness_power)
        * np.power(np.where(radial_ratio > 0, radial_ratio, 1.0), config.omega_radial_power)
    )
    seg_table["sigma_desc_i"] = np.tanh(config.lambda_sigma * seg_table["sigma_i"])
    seg_table["tau_i"] = seg_table["beta_i"] * seg_table["sigma_desc_i"] * seg_table["omega_i"] * seg_table["psi_i"]

    inside = np.clip(1.0 + seg_table["tau_i"], config.min_inside_sqrt, None)
    seg_table["v_desc_segment_kms"] = seg_table["v_bar_internal_segment_kms"] * np.sqrt(inside)
    seg_table["g_bar_internal_km2s2_per_kpc"] = np.square(seg_table["v_bar_internal_segment_kms"]) / np.clip(seg_table["r_mid_kpc"], DEFAULT_EPS, None)
    seg_table["g_desc_km2s2_per_kpc"] = np.square(seg_table["v_desc_segment_kms"]) / np.clip(seg_table["r_mid_kpc"], DEFAULT_EPS, None)
    seg_table["g_transform_km2s2_per_kpc"] = seg_table["g_desc_km2s2_per_kpc"] - seg_table["g_bar_internal_km2s2_per_kpc"]
    ...
    for _, seg in seg_table.iterrows():
        ...
        if local.empty:
            continue
        inside_local = max(1.0 + float(seg["tau_i"]), config.min_inside_sqrt)
        local["galaxy"] = galaxy
        ...
        local["tau_i"] = float(seg["tau_i"])
        local["v_desc_kms"] = local["v_bar_internal_kms"] * math.sqrt(inside_local)
        local["g_bar_internal_km2s2_per_kpc"] = np.square(local["v_bar_internal_kms"]) / np.clip(local["r_kpc"], DEFAULT_EPS, None)
        local["g_desc_km2s2_per_kpc"] = np.square(local["v_desc_kms"]) / np.clip(local["r_kpc"], DEFAULT_EPS, None)
        local["g_transform_km2s2_per_kpc"] = local["g_desc_km2s2_per_kpc"] - local["g_bar_internal_km2s2_per_kpc"]
    ...
    metrics = {
        ...
        "mean_tau_i": float(seg_table["tau_i"].mean()),
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    tau = point_df["tau_i"].to_numpy(dtype=float)
    ...
    fig.suptitle(f"{galaxy} Topostructure V4 Diagnostics", fontsize=14)
    ...
    ax.plot(x, tau, marker="o", linewidth=1.2, label="tau")
    ax.plot(x, omega, marker="o", linewidth=1.2, label="omega")
    ax.set_ylabel("sigma / tau / omega")
    ax.grid(True, alpha=0.3)
    ax2 = ax.twinx()
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        pd.concat(pointwise_frames, ignore_index=True).to_csv(merged_dir / "merged_pointwise_topostructure_v4.csv", index=False)
    if segment_frames:
        pd.concat(segment_frames, ignore_index=True).to_csv(merged_dir / "merged_segment_topostructure_v4.csv", index=False)
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_desc_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(merged_dir / "merged_summary_topostructure_v4.csv", index=False)
...
def write_run_metadata(run_dir: Path, config: ModelConfig, args: argparse.Namespace, input_dir: Path) -> None:
    payload = {
        ...
        "tag": args.tag,
        "dpi": args.dpi,
        "model_note": "V4 inserts a describability transform layer between internal baryonic proxy and external model velocity.",
    }
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure_v4.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure_v4.csv"
            plot_path = plots_dir / f"{slug}_topostructure_v4_diagnostics.png"
    ...
    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        summary_df = summary_df.sort_values(["rmse_desc_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        summary_df.to_csv(run_dir / "topostructure_summary_v4.csv", index=False)
        aggregate = {
            ...
            "mean_tau_i": float(summary_df["mean_tau_i"].mean()),
        }
        (run_dir / "aggregate_metrics_v4.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures_v4.csv", index=False)
