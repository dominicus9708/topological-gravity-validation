...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Topostructure galaxy rotation pipeline v3 with timestamped output, merged CSVs, and plots."
    )
    ...
    parser.add_argument(
        ...
        help="Root directory under which crossmatched/<timestamp>/ will be created. Default: data\\derived",
    )
    ...
    parser.add_argument("--tag", default="topostructure", help="Optional tag to append after timestamp folder name.")
...
def prepare_dataframe(file_path: Path) -> Tuple[pd.DataFrame, str]:
    ...
    if v_bar_col is None:
        ...
        missing_components = [name for name, col in [("v_gas", v_gas_col), ("v_disk", v_disk_col), ("v_bulge", v_bul_col)] if col is None]
    ...
    rename_map = {r_col: "r_kpc", v_obs_col: "v_obs_kms", v_bar_col: "v_bar_kms"}
    if v_err_col is not None:
...
def build_segments_for_galaxy(galaxy: str, df: pd.DataFrame, thickness: Optional[GalaxyThickness], config: ModelConfig):
    ...
    for idx in range(len(edges) - 1):
        ...
        seg_v_bar = local["v_bar_kms"].to_numpy(dtype=float)
    ...
    sigma_scale = float(np.max(np.abs(seg_table["sigma_i"])))
    sigma_scale = sigma_scale if sigma_scale > 0 else 1.0
    ...
    seg_table["g_topo_km2s2_per_kpc"] = seg_table["beta_i"] * seg_table["sigma_i"] * seg_table["g_bar_km2s2_per_kpc"] * seg_table["psi_i"]
    seg_table["g_eff_km2s2_per_kpc"] = seg_table["g_bar_km2s2_per_kpc"] + seg_table["g_topo_km2s2_per_kpc"]
    inside = np.clip(1.0 + seg_table["beta_i"] * seg_table["sigma_i"] * seg_table["psi_i"], config.min_inside_sqrt, None)
    ...
    metrics = {
        ...
        "reduced_chi2_bar_only": safe_reduced_chi2(point_table["v_obs_kms"].to_numpy(), point_table["v_bar_kms"].to_numpy(), point_table["v_err_kms"].to_numpy()),
        "reduced_chi2_topo": safe_reduced_chi2(point_table["v_obs_kms"].to_numpy(), point_table["v_eff_kms"].to_numpy(), point_table["v_err_kms"].to_numpy()),
    }
...
def plot_one_galaxy(point_df: pd.DataFrame, output_path: Path, dpi: int) -> None:
    ...
    v_bar = point_df["v_bar_kms"].to_numpy(dtype=float)
    v_eff = point_df["v_eff_kms"].to_numpy(dtype=float)
    resid_bar = point_df["rotation_residual_bar_only_kms"].to_numpy(dtype=float)
    resid_topo = point_df["rotation_residual_kms"].to_numpy(dtype=float)
...
def merge_outputs(pointwise_frames: List[pd.DataFrame], segment_frames: List[pd.DataFrame], summary_rows: List[Dict[str, float]], merged_dir: Path) -> None:
    ...
    if pointwise_frames:
        merged_pointwise = pd.concat(pointwise_frames, ignore_index=True)
        merged_pointwise.to_csv(merged_dir / "merged_pointwise_topostructure.csv", index=False)
    if segment_frames:
        merged_segment = pd.concat(segment_frames, ignore_index=True)
        merged_segment.to_csv(merged_dir / "merged_segment_topostructure.csv", index=False)
    if summary_rows:
        merged_summary = pd.DataFrame(summary_rows)
        merged_summary = merged_summary.sort_values(["rmse_topo_kms", "rmse_improvement_kms"], ascending=[True, False]).reset_index(drop=True)
        merged_summary.to_csv(merged_dir / "merged_summary_topostructure.csv", index=False)
...
def main() -> None:
    ...
    for file_path in files:
        try:
            ...
            seg_path = per_galaxy_dir / f"{slug}_segment_structure.csv"
            point_path = per_galaxy_dir / f"{slug}_pointwise_topostructure.csv"
            plot_path = plots_dir / f"{slug}_topostructure_diagnostics.png"
    ...
    if summary_rows:
        ...
        summary_df.to_csv(run_dir / "topostructure_summary.csv", index=False)
        aggregate = {
        ...
        (run_dir / "aggregate_metrics.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    if failure_rows:
        pd.DataFrame(failure_rows).to_csv(run_dir / "topostructure_failures.csv", index=False)
