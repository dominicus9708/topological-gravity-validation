...
def build_shell_distribution_summary(df: pd.DataFrame, dataset_name: str, is_6d: bool) -> pd.DataFrame:
    ...
    for shell in SHELL_LABELS:
        ...
        if len(vals) == 0:
            rows.append({
                "dataset": dataset_name, "shell": shell, "n_shell": int(len(sub)), "metric_col": metric_col,
        ...
        rows.append({
            "dataset": dataset_name, "shell": shell, "n_shell": int(len(sub)), "metric_col": metric_col,
            "obs_n_valid": int(len(vals)),
            "obs_median": float(np.nanmedian(vals)),
            "obs_q10": float(np.nanquantile(vals, 0.10)),
            "obs_q25": float(np.nanquantile(vals, 0.25)),
            "obs_q75": float(np.nanquantile(vals, 0.75)),
            "obs_q90": float(np.nanquantile(vals, 0.90)),
...
def build_shell_summary(project_root: Path, df: pd.DataFrame, dataset_name: str, is_6d: bool, standard_shell_df: pd.DataFrame | None, standard_source_path: str) -> pd.DataFrame:
    ...
    for shell in SHELL_LABELS:
        ...
        rows.append({
            "dataset": dataset_name, "shell": shell, "shell_center_kpc": SHELL_CENTERS[shell], "n_shell": int(n),
            "D_bg_reference": D_BG_REFERENCE, "D_halo_shell": d_halo, "Sigma_bg_shell": sigma_bg,
            "sigma_local_mean_shell": sigma_local_mean, "sigma_local_spread_shell": sigma_local_spread,
            "Sigma_shell_effective": sigma_shell_effective, "lambda_structural_spread": LAMBDA_STRUCTURAL_SPREAD,
            "observed_series_source": observed_source, "standard_series_source": standard_model_source,
            "standard_source_path": standard_source_path, "observed_variance_shell": observed_var,
            "standard_variance_shell": standard_var, "residual_observed_minus_standard": residual
        })
...
def summarize_dataset(df: pd.DataFrame, shell_df: pd.DataFrame, dataset_name: str) -> dict:
    ...
    return {
        "dataset": dataset_name, "rows_read": int(len(df)), "rows_written": int(len(df)),
...
def plot_shell_interval_comparison(dist_df: pd.DataFrame, shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    merged = dist_df.merge(
        shell_df[["shell", "Sigma_shell_effective", "topological_gradient_term_effective"]],
        on="shell", how="left"
    )
    ...
    obs_outer_color = "tab:blue"
    obs_inner_color = "tab:orange"
    obs_median_color = "tab:red"
    topo_contrast_color = "tab:purple"
    topo_gradient_color = "tab:green"

    m = np.isfinite(x) & np.isfinite(obs_median)
    if m.any():
        for xi, q10, q25, med, q75, q90 in zip(x[m], obs_q10[m], obs_q25[m], obs_median[m], obs_q75[m], obs_q90[m]):
            if np.isfinite(q10) and np.isfinite(q90):
                axes[0].vlines(xi, q10, q90, linewidth=10, alpha=0.22, color=obs_outer_color)
            if np.isfinite(q25) and np.isfinite(q75):
                axes[0].vlines(xi, q25, q75, linewidth=18, alpha=0.32, color=obs_inner_color)
            axes[0].scatter([xi], [med], s=90, marker="o", color=obs_median_color, zorder=3, label="_nolegend_")
    axes[0].plot([], [], linewidth=10, alpha=0.22, color=obs_outer_color, label="Observed 10-90% interval")
    axes[0].plot([], [], linewidth=18, alpha=0.32, color=obs_inner_color, label="Observed 25-75% interval")
    axes[0].scatter([], [], s=90, marker="o", color=obs_median_color, label="Observed median")
    axes[0].set_ylabel("Observed speed proxy scale")
    axes[0].set_title(f"{dataset_name.upper()} observation interval comparison")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()

    m_sig = np.isfinite(x) & np.isfinite(sig_n)
    m_grad = np.isfinite(x) & np.isfinite(grad_n)
    if m_sig.any():
        for xi, yi in zip(x[m_sig], sig_n[m_sig]):
            axes[1].vlines(xi - 0.9, 0.0, yi, linewidth=14, alpha=0.30, color=topo_contrast_color)
            axes[1].scatter([xi - 0.9], [yi], s=95, marker="o", color=topo_contrast_color, zorder=3, label="_nolegend_")
    if m_grad.any():
        for xi, yi in zip(x[m_grad], grad_n[m_grad]):
            axes[1].vlines(xi + 0.9, 0.0, yi, linewidth=8, alpha=0.45, color=topo_gradient_color)
            axes[1].scatter([xi + 0.9], [yi], s=95, marker="s", color=topo_gradient_color, zorder=3, label="_nolegend_")
    axes[1].plot([], [], linewidth=14, alpha=0.30, color=topo_contrast_color, label="Topological effective contrast")
    axes[1].plot([], [], linewidth=8, alpha=0.45, color=topo_gradient_color, label="Topological effective gradient")
    axes[1].scatter([], [], s=95, marker="o", color=topo_contrast_color, label="Effective contrast value")
    axes[1].scatter([], [], s=95, marker="s", color=topo_gradient_color, label="Gradient value")
    axes[1].set_ylabel("Normalized topological level")
    axes[1].set_xlabel("Shell center [kpc]")
    axes[1].set_title(f"{dataset_name.upper()} topological interval-style output")
...
def plot_topological_only(shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    x, shells = shell_x_and_labels(shell_df)
    sig_n = robust_normalize(safe_numeric(shell_df["Sigma_shell_effective"]).to_numpy())
    grad_n = robust_normalize(safe_numeric(shell_df["topological_gradient_term_effective"]).to_numpy())
    ...
    if m_sig.any():
        for xi, yi in zip(x[m_sig], sig_n[m_sig]):
            ax.vlines(xi - 1.0, 0.0, yi, linewidth=14, alpha=0.30, color=topo_contrast_color)
            ax.scatter([xi - 1.0], [yi], s=95, marker="o", color=topo_contrast_color, zorder=3, label="_nolegend_")
    if m_grad.any():
        for xi, yi in zip(x[m_grad], grad_n[m_grad]):
            ax.vlines(xi + 1.0, 0.0, yi, linewidth=8, alpha=0.45, color=topo_gradient_color)
            ax.scatter([xi + 1.0], [yi], s=95, marker="s", color=topo_gradient_color, zorder=3, label="_nolegend_")
    ax.plot([], [], linewidth=14, alpha=0.30, color=topo_contrast_color, label="Topological effective contrast")
    ax.plot([], [], linewidth=8, alpha=0.45, color=topo_gradient_color, label="Topological effective gradient")
    ax.scatter([], [], s=95, marker="o", color=topo_contrast_color, label="Effective contrast value")
    ax.scatter([], [], s=95, marker="s", color=topo_gradient_color, label="Gradient value")
    ax.set_ylabel("Normalized topological level")
    ax.set_xlabel("Shell center [kpc]")
    ax.set_title(f"{dataset_name.upper()} topological-only output")
...
def run_one_dataset(project_root: Path, dataset_name: str, is_6d: bool, output_dir: Path, plot_dir: Path) -> dict:
    ...
    shell_df = build_shell_summary(
        project_root=project_root, df=df, dataset_name=dataset_name, is_6d=is_6d,
        standard_shell_df=standard_shell_df, standard_source_path=standard_source_path
    )
    ...
    plot_shell_interval_comparison(dist_df, shell_df, dataset_name, plot_dir / f"{dataset_name}_shell_interval_comparison.png")
    plot_topological_only(shell_df, dataset_name, plot_dir / f"{dataset_name}_topological_only.png")
...
def main() -> None:
    ...
    readme = '''Our galaxy Halo Stellar Kinematics - topological stage 017

Changes from stage 016
- Standard-only distance panels are removed.
- Observation and topological interval plots now use clearly separated color families.
- 6D observation panel uses shell-wise vertical intervals rather than continuous fill.
- Only the presentation-relevant plots are saved.
'''
    ...
    print(f" - {plot_dir / '5d_shell_interval_comparison.png'}")
    print(f" - {plot_dir / '5d_topological_only.png'}")
    print(f" - {plot_dir / '5d_structure_support.png'}")
    print(f" - {plot_dir / '5d_complexity_support.png'}")
    print(f" - {plot_dir / '6d_shell_interval_comparison.png'}")
    print(f" - {plot_dir / '6d_topological_only.png'}")
