...
# Topological pipeline 004
# ------------------------------------------------------------
# Placement:
#   src/Our galaxy Halo Stellar Kinematics/topological/
#     run_topological_our_galaxy_halo_rrlyrae_004.py
#
# Input:
...
# - The main structural driver is coarse-grained shell structure:
...
# - Stabilization logic remains active for proxy control and
#   shell-gradient reliability.
...
def build_shell_summary(df: pd.DataFrame, config: DatasetConfig) -> pd.DataFrame:
    ...
    coupling_diag_valid = np.full(len(shell_df), False, dtype=bool)
    for i in range(len(shell_df)):
        valid = (
            np.isfinite(grad_bg[i]) and grad_bg[i] > EPS
            and np.isfinite(obs_var[i]) and np.isfinite(std_var[i])
            and np.isfinite(n_shell[i]) and n_shell[i] >= MIN_SHELL_N_FOR_COUPLING_DIAG
        )
        if valid:
            coupling_diag[i] = (obs_var[i] - std_var[i]) / grad_bg[i]
            coupling_diag_valid[i] = True

    shell_df["topological_gradient_term_bg"] = grad_bg
    shell_df["coupling_shell_diagnostic"] = coupling_diag
    shell_df["coupling_shell_diagnostic_valid"] = coupling_diag_valid
    shell_df["residual_observed_minus_standard"] = np.where(
        np.isfinite(obs_var) & np.isfinite(std_var),
        obs_var - std_var,
        np.nan,
    )
    return shell_df
...
def summarize_dataset(df: pd.DataFrame, shell_df: pd.DataFrame, config: DatasetConfig) -> dict:
    coupling_diag = pd.to_numeric(shell_df.get("coupling_shell_diagnostic"), errors="coerce")
    coupling_valid = shell_df.get("coupling_shell_diagnostic_valid")
    if coupling_valid is None:
        coupling_valid_n = 0
    else:
        coupling_valid_n = int(pd.Series(coupling_valid).fillna(False).astype(bool).sum())
    ...
    return {
        ...
        "median_coupling_shell_diagnostic": float(np.nanmedian(coupling_diag)) if len(coupling_diag) > 0 else np.nan,
        "coupling_shell_diagnostic_valid_n": coupling_valid_n,
    }
...
def run_one_dataset(project_root: Path, config: DatasetConfig, output_dir: Path) -> dict:
    ...
    shell_to_cdiag = dict(zip(shell_df["shell"], shell_df["coupling_shell_diagnostic"]))
    shell_to_cvalid = dict(zip(shell_df["shell"], shell_df["coupling_shell_diagnostic_valid"]))
    ...
    df["coupling_shell_diagnostic"] = df["topological_shell"].astype(str).map(shell_to_cdiag)
    df["coupling_shell_diagnostic_valid"] = df["topological_shell"].astype(str).map(shell_to_cvalid)
...
def main() -> None:
    ...
    readme = (
        ...
        f"- MIN_SHELL_N_FOR_COUPLING_DIAG = {MIN_SHELL_N_FOR_COUPLING_DIAG}\n"
    )
