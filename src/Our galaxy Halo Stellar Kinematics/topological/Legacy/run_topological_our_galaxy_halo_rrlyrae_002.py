...
# Topological pipeline 002
# ------------------------------------------------------------
# Placement:
#   src/Our galaxy Halo Stellar Kinematics/topological/
#     run_topological_our_galaxy_halo_rrlyrae_002.py
#
# Input:
...
# - This revision stabilizes derived spikes and prepares
#   xi*c_info^2 derivation-ready shell quantities.
# ============================================================
...
def add_proxy_columns(df: pd.DataFrame, is_6d: bool) -> pd.DataFrame:
    ...
    # Distance proxy stabilization is applied only to derived columns.
...
def build_shell_summary(df: pd.DataFrame, config: DatasetConfig) -> pd.DataFrame:
    ...
    for shell in SHELL_LABELS:
        ...
        if n == 0:
            rows.append({
                ...
                "xi_cinfo2_shell_estimate": np.nan,
            })
            continue
    ...
    xi_est = np.full(len(shell_df), np.nan, dtype=float)
    ...
    for i in range(len(shell_df)):
        ...
        if np.isfinite(grad[i]) and grad[i] > EPS and np.isfinite(obs_var[i]) and np.isfinite(std_var[i]):
            xi_est[i] = (obs_var[i] - std_var[i]) / grad[i]

    shell_df["topological_gradient_term"] = grad
    shell_df["topological_prediction_norm"] = pred_norm
    shell_df["xi_cinfo2_shell_estimate"] = xi_est
    return shell_df
...
def summarize_dataset(df: pd.DataFrame, shell_df: pd.DataFrame, config: DatasetConfig) -> dict:
    return {
        ...
        "median_xi_cinfo2_shell_estimate": float(np.nanmedian(pd.to_numeric(shell_df["xi_cinfo2_shell_estimate"], errors="coerce"))) if "xi_cinfo2_shell_estimate" in shell_df.columns else np.nan,
    }
...
def main() -> None:
    ...
    readme = (
        "Our galaxy Halo Stellar Kinematics - topological stage 002\n\n"
        "Changes in this revision\n"
        "- Output is written into a timestamped subfolder.\n"
        "- Derived spike stabilization is applied only to proxy columns.\n"
        "- xi*c_info^2 shell-estimate-ready column is added.\n"
