...
# Topological pipeline 007
# ------------------------------------------------------------
# Placement:
#   src/Our galaxy Halo Stellar Kinematics/topological/
#     run_topological_our_galaxy_halo_rrlyrae_007.py
#
# Input:
...
def find_latest_existing_file(base_dir: Path, filename: str) -> Path:
...
def run_one_dataset(project_root: Path, config: DatasetConfig, output_dir: Path) -> dict:
    ...
    standard_path = find_latest_existing_file(standard_dir, config.standard_filename)
...
def main() -> None:
    ...
    readme = """Our galaxy Halo Stellar Kinematics - topological stage 007
