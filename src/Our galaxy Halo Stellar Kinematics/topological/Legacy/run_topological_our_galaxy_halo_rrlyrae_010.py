...
def attach_standard_shell_model(dataset_name: str, standard_dir: Path, is_6d: bool) -> tuple[pd.DataFrame | None, str]:
...
def get_standard_model_map(standard_shell_df: pd.DataFrame | None, is_6d: bool) -> tuple[dict, str]:
    if standard_shell_df is None:
        return {}, ""
    col = "standard_speed_variance_shell_model" if is_6d else "standard_vt_variance_shell_model"
    if col not in standard_shell_df.columns:
        return {}, ""
    mp = dict(zip(standard_shell_df["shell"].astype(str), safe_numeric(standard_shell_df[col])))
    return mp, col
...
def build_shell_summary(df: pd.DataFrame, dataset_name: str, is_6d: bool, standard_shell_df: pd.DataFrame | None, standard_source_path: str) -> pd.DataFrame:
    rows = []
    standard_map, standard_model_source = get_standard_model_map(standard_shell_df, is_6d=is_6d)
...
def run_one_dataset(project_root: Path, dataset_name: str, is_6d: bool, output_dir: Path) -> dict:
    ...
    standard_shell_df, standard_source_path = attach_standard_shell_model(dataset_name, standard_dir, is_6d=is_6d)
...
def main() -> None:
    ...
    readme = (
        "Our galaxy Halo Stellar Kinematics - topological stage 010\n\n"
        "Changes in this revision\n"
        "- Robust structural aggregation is retained.\n"
        "- Standard comparison is read from shell summary model files in results/.../standard.\n"
        "- Comparison target is shell-level variance model rather than raw standard proxy values.\n"
