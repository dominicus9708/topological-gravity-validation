...
def robust_normalize(y: pd.Series | np.ndarray) -> np.ndarray:
...
def attach_standard_shell_data(dataset_name: str, standard_dir: Path, is_6d: bool):
...
def choose_observed_series(sub: pd.DataFrame, is_6d: bool):
...
def get_standard_maps(standard_shell_df: pd.DataFrame | None, is_6d: bool):
...
def build_shell_distribution_summary(df: pd.DataFrame, dataset_name: str, is_6d: bool) -> pd.DataFrame:
    rows = []
    metric_col = "speed_total_proxy_kms_soft" if is_6d else "vt_total_proxy_kms_soft"
    for shell in SHELL_LABELS:
        sub = df[df["topological_shell"].astype(str) == shell].copy()
        vals = safe_numeric(sub.get(metric_col)).dropna().to_numpy()
        if len(vals) == 0:
            rows.append({"dataset": dataset_name, "shell": shell, "n_shell": int(len(sub)), "metric_col": metric_col,
                         "obs_n_valid": 0, "obs_median": np.nan, "obs_q10": np.nan, "obs_q25": np.nan,
                         "obs_q75": np.nan, "obs_q90": np.nan, "obs_var": np.nan})
            continue
        rows.append({"dataset": dataset_name, "shell": shell, "n_shell": int(len(sub)), "metric_col": metric_col,
                     "obs_n_valid": int(len(vals)), "obs_median": float(np.nanmedian(vals)),
                     "obs_q10": float(np.nanquantile(vals, 0.10)), "obs_q25": float(np.nanquantile(vals, 0.25)),
                     "obs_q75": float(np.nanquantile(vals, 0.75)), "obs_q90": float(np.nanquantile(vals, 0.90)),
                     "obs_var": float(np.nanvar(vals, ddof=1)) if len(vals) > 1 else np.nan})
...
def classify_stream_richness(counts: pd.Series) -> pd.Series:
    ...
    labels = []
    for v in x:
        labels.append("low" if v <= q1 else "medium" if v <= q2 else "high")
...
def build_stream_shell_diag_from_input(project_root: Path) -> pd.DataFrame:
    ...
    if not overlay_path.exists():
        for shell in SHELL_LABELS:
            rows.append({"shell": shell, "n_stream_points_overlap": np.nan, "n_unique_tracks_overlap": np.nan,
                         "n_unique_streams_overlap": np.nan, "stream_richness_flag": pd.NA})
        return pd.DataFrame(rows)
...
def build_shell_summary(project_root: Path, df: pd.DataFrame, dataset_name: str, is_6d: bool, standard_shell_df: pd.DataFrame | None, standard_source_path: str) -> pd.DataFrame:
    ...
    for shell in SHELL_LABELS:
        ...
        sigma_shell_effective = sigma_bg + LAMBDA_STRUCTURAL_SPREAD * sigma_local_spread if np.isfinite(sigma_bg) and np.isfinite(sigma_local_spread) else np.nan
        ...
        rows.append({"dataset": dataset_name, "shell": shell, "shell_center_kpc": SHELL_CENTERS[shell],
                     "n_shell": int(n), "D_bg_reference": D_BG_REFERENCE, "D_halo_shell": d_halo,
                     "Sigma_bg_shell": sigma_bg, "sigma_local_mean_shell": sigma_local_mean,
                     "sigma_local_spread_shell": sigma_local_spread, "Sigma_shell_effective": sigma_shell_effective,
                     "lambda_structural_spread": LAMBDA_STRUCTURAL_SPREAD, "observed_series_source": observed_source,
                     "standard_series_source": standard_model_source, "standard_source_path": standard_source_path,
                     "observed_variance_shell": observed_var, "standard_variance_shell": standard_var,
                     "residual_observed_minus_standard": residual})

    shell_df = pd.DataFrame(rows).merge(stream_diag, on="shell", how="left")
    ...
    for i in range(len(shell_df)):
        if n_shell[i] >= MIN_SHELL_N_FOR_COUPLING_DIAG and np.isfinite(grad_eff[i]) and grad_eff[i] > EPS and np.isfinite(residual[i]):
...
def summarize_dataset(df: pd.DataFrame, shell_df: pd.DataFrame, dataset_name: str) -> dict:
    ...
    return {"dataset": dataset_name, "rows_read": int(len(df)), "rows_written": int(len(df)),
...
def plot_observation_first(dist_df: pd.DataFrame, shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    merged = dist_df.merge(shell_df[["shell", "standard_variance_shell", "Sigma_shell_effective", "topological_gradient_term_effective"]], on="shell", how="left")
    x, shells = shell_x_and_labels(merged)
    obs_median = safe_numeric(merged["obs_median"]).to_numpy()
    obs_q10 = safe_numeric(merged["obs_q10"]).to_numpy()
    obs_q25 = safe_numeric(merged["obs_q25"]).to_numpy()
    obs_q75 = safe_numeric(merged["obs_q75"]).to_numpy()
    obs_q90 = safe_numeric(merged["obs_q90"]).to_numpy()
    obs_var = safe_numeric(merged["obs_var"]).to_numpy()
    std_var = safe_numeric(merged["standard_variance_shell"]).to_numpy()
    obs_var_n = robust_normalize(obs_var)
    std_var_n = robust_normalize(std_var)
    topo_sig_n = robust_normalize(safe_numeric(merged["Sigma_shell_effective"]).to_numpy())
    topo_grad_n = robust_normalize(safe_numeric(merged["topological_gradient_term_effective"]).to_numpy())

    fig, axes = plt.subplots(2, 1, figsize=(10, 9), sharex=True)
    m = np.isfinite(x)
    if m.any():
        axes[0].fill_between(x[m], obs_q10[m], obs_q90[m], alpha=0.18, label="Observed 10-90% band")
        axes[0].fill_between(x[m], obs_q25[m], obs_q75[m], alpha=0.30, label="Observed 25-75% band")
        axes[0].scatter(x[m], obs_median[m], s=60, marker="o", label="Observed median")
    m_std = np.isfinite(x) & np.isfinite(std_var)
    if m_std.any():
        axes[0].scatter(x[m_std], np.sqrt(std_var[m_std]), s=65, marker="s", label="Standard shell sigma (sqrt variance)")
    axes[0].set_ylabel("Observed speed proxy scale")
    axes[0].set_title(f"{dataset_name.upper()} observation-first shell comparison")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()

    m_obsn = np.isfinite(x) & np.isfinite(obs_var_n)
    m_stdn = np.isfinite(x) & np.isfinite(std_var_n)
    m_sig = np.isfinite(x) & np.isfinite(topo_sig_n)
    m_grad = np.isfinite(x) & np.isfinite(topo_grad_n)
    if m_obsn.any():
        axes[1].scatter(x[m_obsn], obs_var_n[m_obsn], s=60, marker="o", label="Observed variance trend (normalized)")
    if m_stdn.any():
        axes[1].scatter(x[m_stdn], std_var_n[m_stdn], s=60, marker="s", label="Standard variance trend (normalized)")
    if m_sig.any():
        axes[1].scatter(x[m_sig], topo_sig_n[m_sig], s=60, marker="^", label="Topological effective contrast trend")
    if m_grad.any():
        axes[1].scatter(x[m_grad], topo_grad_n[m_grad], s=60, marker="D", label="Topological effective gradient trend")
    axes[1].set_ylabel("Normalized trend")
    axes[1].set_xlabel("Shell center [kpc]")
    axes[1].set_title(f"{dataset_name.upper()} trend-level consistency comparison")
...
def plot_distance_to_observation(dist_df: pd.DataFrame, shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    merged = dist_df.merge(shell_df[["shell", "standard_variance_shell", "Sigma_shell_effective", "topological_gradient_term_effective"]], on="shell", how="left")
    x, shells = shell_x_and_labels(merged)
    obs_var = safe_numeric(merged["obs_var"]).to_numpy()
    std_var = safe_numeric(merged["standard_variance_shell"]).to_numpy()
    std_abs = np.abs(std_var - obs_var)
    obs_n = robust_normalize(obs_var)
    sig_n = robust_normalize(safe_numeric(merged["Sigma_shell_effective"]).to_numpy())
    grad_n = robust_normalize(safe_numeric(merged["topological_gradient_term_effective"]).to_numpy())
    topo_sig_abs = np.abs(sig_n - obs_n)
    topo_grad_abs = np.abs(grad_n - obs_n)

    fig, axes = plt.subplots(2, 1, figsize=(10, 8), sharex=True)
    m_std = np.isfinite(x) & np.isfinite(std_abs)
    if m_std.any():
        axes[0].bar(x[m_std], std_abs[m_std], width=5.0, label="|standard variance - observed variance|")
    axes[0].set_yscale("log")
    axes[0].set_ylabel("Absolute distance (log)")
    axes[0].set_title(f"{dataset_name.upper()} direct Newtonian distance to observation")
    axes[0].grid(True, axis="y", alpha=0.3)
    axes[0].legend()

    m_sig = np.isfinite(x) & np.isfinite(topo_sig_abs)
    m_grad = np.isfinite(x) & np.isfinite(topo_grad_abs)
    if m_sig.any():
        axes[1].bar(x[m_sig] - 1.5, topo_sig_abs[m_sig], width=3.0, label="|topological effective contrast trend - observed trend|")
    if m_grad.any():
        axes[1].bar(x[m_grad] + 1.5, topo_grad_abs[m_grad], width=3.0, label="|topological gradient trend - observed trend|")
    axes[1].set_ylabel("Trend distance")
    axes[1].set_xlabel("Shell center [kpc]")
    axes[1].set_title(f"{dataset_name.upper()} trend-level topological distance to observation")
...
def plot_structure_support(shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    ...
    axes[0].set_title(f"{dataset_name.upper()} structural background support")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()
    ...
    axes[1].set_title(f"{dataset_name.upper()} shell-local structural spread")
    axes[1].grid(True, alpha=0.3)
    axes[1].legend()
    ...
    axes[2].set_title(f"{dataset_name.upper()} structural gradient support")
...
def plot_complexity_support(shell_df: pd.DataFrame, dataset_name: str, outpath: Path) -> None:
    x, shells = shell_x_and_labels(shell_df)
...
def run_one_dataset(project_root: Path, dataset_name: str, is_6d: bool, output_dir: Path, plot_dir: Path) -> dict:
    ...
    dist_df = build_shell_distribution_summary(df, dataset_name=dataset_name, is_6d=is_6d)
    standard_shell_df, standard_source_path = attach_standard_shell_data(dataset_name, standard_dir, is_6d=is_6d)
    shell_df = build_shell_summary(project_root=project_root, df=df, dataset_name=dataset_name, is_6d=is_6d, standard_shell_df=standard_shell_df, standard_source_path=standard_source_path)
    ...
    df.to_csv(output_dir / f"gaia_rrlyrae_{dataset_name}_topological.csv", index=False)
    shell_df.to_csv(output_dir / f"gaia_rrlyrae_{dataset_name}_topological_shells.csv", index=False)
    dist_df.to_csv(output_dir / f"gaia_rrlyrae_{dataset_name}_observed_shell_distribution.csv", index=False)

    plot_observation_first(dist_df, shell_df, dataset_name, plot_dir / f"{dataset_name}_observation_first.png")
    plot_distance_to_observation(dist_df, shell_df, dataset_name, plot_dir / f"{dataset_name}_distance_to_observation.png")
    plot_structure_support(shell_df, dataset_name, plot_dir / f"{dataset_name}_structure_support.png")
    plot_complexity_support(shell_df, dataset_name, plot_dir / f"{dataset_name}_complexity_support.png")
...
def main() -> None:
    ...
    readme = (
        "Our galaxy Halo Stellar Kinematics - topological stage 016\n\n"
        "Main purpose\n"
        "- Make visual comparison with observation immediate.\n\n"
        "Important note\n"
        "- The current topological stage still does not output a same-unit predicted shell variance.\n"
        "- Therefore the observation-first figure overlays observed shell distribution with the standard shell representative directly,\n"
        "  while the topological quantity is shown as a trend-alignment signal in a second panel.\n\n"
        "Generated plots\n"
        "- observation_first: observed shell distribution bands + standard shell sigma + normalized topological trend panel\n"
        "- distance_to_observation: direct Newtonian distance to observed variance + topological trend-distance panel\n"
        "- structure_support: structural interpretation support\n"
        "- complexity_support: shell complexity and caution support\n"
    )
    ...
    print(f" - {output_dir / 'gaia_rrlyrae_5d_observed_shell_distribution.csv'}")
    print(f" - {output_dir / 'gaia_rrlyrae_6d_topological.csv'}")
    print(f" - {output_dir / 'gaia_rrlyrae_6d_topological_shells.csv'}")
    print(f" - {output_dir / 'gaia_rrlyrae_6d_observed_shell_distribution.csv'}")
    print(f" - {output_dir / 'topological_summary.csv'}")
    print(f" - {plot_dir / '5d_observation_first.png'}")
    print(f" - {plot_dir / '5d_distance_to_observation.png'}")
    print(f" - {plot_dir / '5d_structure_support.png'}")
    print(f" - {plot_dir / '5d_complexity_support.png'}")
    print(f" - {plot_dir / '6d_observation_first.png'}")
    print(f" - {plot_dir / '6d_distance_to_observation.png'}")
    print(f" - {plot_dir / '6d_structure_support.png'}")
    print(f" - {plot_dir / '6d_complexity_support.png'}")
