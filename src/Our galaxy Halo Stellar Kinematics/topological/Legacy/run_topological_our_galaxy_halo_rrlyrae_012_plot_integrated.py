    ...
    for shell in SHELL_LABELS:
        ...
        if n == 0:
            rows.append({
                ...
                "sigma_local_spread_shell": np.nan,
                "Sigma_shell_effective": np.nan,
        ...
        sigma_local_spread = weighted_mad(sigma_local, q, center=0.0)
...
def write_readme(path: Path, source_dir: Path, plot_dir: Path) -> None:
    text = (
        "Our galaxy Halo Stellar Kinematics - topological stage 012+013 integrated\n\n"
        f"Output folder\n- {source_dir}\n\n"
        f"Plot folder\n- {plot_dir}\n\n"
        "Generated figures\n"
        ...
        "- Structural response plots show D_halo_shell, Sigma_bg_shell, sigma_local_spread_shell, Sigma_shell_effective, and effective topological gradient.\n"
...
def main() -> None:
    ...
    shell_5d = read_csv(output_dir / "gaia_rrlyrae_5d_topological_shells.csv")
    shell_6d = read_csv(output_dir / "gaia_rrlyrae_6d_topological_shells.csv")
    ...
    readme = (
        ...
        "- Stage 012 shell-effective structural spread calculation is retained.\n"
        "- Plot generation is integrated into the same run to avoid Legacy-folder lookup mistakes.\n"
        "- A plot subfolder is created inside the same timestamped output folder.\n"
    )
    (output_dir / "README_topological.txt").write_text(readme, encoding="utf-8")
    write_readme(output_dir / "README_plots.txt", output_dir, plot_dir)
    ...
    print(f" - {output_dir / 'README_plots.txt'}")
