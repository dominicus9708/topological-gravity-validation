...
def attach_standard_columns(df: pd.DataFrame, standard_dir: Path, dataset_name: str) -> tuple[pd.DataFrame, str]:
    out = df.copy()
    standard_path = standard_dir / f"gaia_rrlyrae_{dataset_name}_standard.csv"
    if not standard_path.exists():
        out["standard_available"] = False
        return out, ""

    std = read_csv(standard_path)
    keep = ["source_id"] + [c for c in std.columns if c != "source_id"]
    std = std[keep].copy()
...
def choose_standard_series(sub: pd.DataFrame, is_6d: bool) -> tuple[pd.Series, str]:
    if is_6d:
        preferred = [
            ...
            "std_standard_radial_velocity_kms",
...
def build_shell_summary(df: pd.DataFrame, dataset_name: str, is_6d: bool, standard_source_path: str) -> pd.DataFrame:
    ...
    for shell in SHELL_LABELS:
        ...
        observed_series, observed_source = choose_observed_series(sub, is_6d)
        standard_series, standard_source = choose_standard_series(sub, is_6d)
...
def run_one_dataset(project_root: Path, dataset_name: str, is_6d: bool, output_dir: Path) -> dict:
    ...
    shell_df = build_shell_summary(df, dataset_name, is_6d, standard_source_path)
...
def main() -> None:
    ...
    readme = (
        "Our galaxy Halo Stellar Kinematics - topological stage 009\n\n"
        "Changes in this revision\n"
        "- Robust structural aggregation is retained.\n"
        "- Standard comparison is read directly from results/Our galaxy Halo Stellar Kinematics/output/standard.\n"
        "- Non-centered baseline columns are forced as preferred comparison targets.\n"
        "- Centered standard columns remain final fallback only.\n"
