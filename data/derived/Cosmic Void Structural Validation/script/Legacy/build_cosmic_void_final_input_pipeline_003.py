...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--core-representative-count",
        type=int,
        default=4,
        help="Number of core representatives to export.",
    )
    parser.add_argument(
        "--support-representative-count",
        type=int,
        default=2,
        help="Number of support representatives to export.",
    )
    parser.add_argument(
        "--stacked-core-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Row chunk size when scanning the stellar mass FITS.",
    )
    parser.add_argument(
        "--candidate-pool-size",
        type=int,
        default=300,
        help="Number of top-scored void candidates to audit for mass matches.",
    )
    parser.add_argument(
        "--run-tag",
        default="",
...
def get_stellar_columns(stellar_path: Path) -> Tuple[object, Dict[str, str], int]:
...
def prepare_candidate_pool(scored: pd.DataFrame, preferred_algorithms: List[str], candidate_pool_size: int) -> pd.DataFrame:
    ...
    # Prefer core algorithms and higher score, but allow all algorithms in pool.
...
def scan_stellar_matches(
    ...
    # median not tracked in streaming mode; use mean as safe placeholder only if matched.
...
def diversify_select(df: pd.DataFrame, target_count: int) -> pd.DataFrame:
    ...
    pool = df.sort_values(["selection_score", "mass_match_count", "radius_eff"], ascending=[False, False, False]).copy()
    chosen = []
...
def build_core_representatives(candidates: pd.DataFrame, preferred_algorithms: List[str], count: int) -> pd.DataFrame:
    ...
    core = add_input_metadata(core, "core_representative", "mass_augmented_candidate_selection")
...
def build_support_representatives(candidates: pd.DataFrame, preferred_algorithms: List[str], count: int) -> pd.DataFrame:
    ...
    support = add_input_metadata(support, "support_representative", "mass_augmented_candidate_selection")
...
def build_core_stacked(candidates: pd.DataFrame, preferred_algorithms: List[str], size: int) -> pd.DataFrame:
    ...
    core = core.sort_values(["selection_score", "mass_match_count", "radius_eff"], ascending=[False, False, False]).head(size).copy()
    core = add_input_metadata(core, "core_stacked", "mass_augmented_candidate_selection")
...
def build_support_stacked(candidates: pd.DataFrame, preferred_algorithms: List[str], size: int) -> pd.DataFrame:
    ...
    support = support.sort_values(["selection_score", "mass_match_count", "radius_eff"], ascending=[False, False, False]).head(size).copy()
    support = add_input_metadata(support, "support_stacked", "mass_augmented_candidate_selection")
...
def write_summary(
    ...
    lines = []
    lines.append("Cosmic Void final input builder summary (003 mass-eligible reselection)")
    lines.append("===================================================================")
    ...
    lines.append("-------------------")
    lines.append(
        "This 003 stage does not preserve the old 37 rows by force. "
        "Instead, it re-audits the top-scored void candidates against the DESI stellar-mass FITS in streaming mode, "
        "then rebuilds the input files using only mass-eligible matched voids."
    )
...
def main() -> int:
    ...
    _, stellar_meta, _ = get_stellar_columns(stellar_path)
    ...
    core_rep = harmonize_columns(build_core_representatives(matched, preferred_algorithms, args.core_representative_count))
    support_rep = harmonize_columns(build_support_representatives(matched, preferred_algorithms, args.support_representative_count))
    core_stacked = harmonize_columns(build_core_stacked(matched, preferred_algorithms, args.stacked_core_size))
    support_stacked = harmonize_columns(build_support_stacked(matched, preferred_algorithms, args.stacked_support_size))
    ...
    # Also save audit table for transparency
