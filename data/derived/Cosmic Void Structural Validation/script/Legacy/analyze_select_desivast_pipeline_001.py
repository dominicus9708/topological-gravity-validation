...
def standardize_catalog(df: pd.DataFrame, ctx: CatalogContext) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    standardized["radius_rank_pct"] = standardized["radius_eff"].rank(pct=True)
    standardized["z_rank_pct"] = standardized["z"].rank(pct=True)
...
def add_selection_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    nm = pd.to_numeric(out["n_members"], errors="coerce")
