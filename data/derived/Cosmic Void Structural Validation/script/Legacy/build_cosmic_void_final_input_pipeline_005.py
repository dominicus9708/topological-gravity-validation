...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--match-radius-deg", type=float, default=0.1)
    parser.add_argument("--max-nearest-sep-deg", type=float, default=0.01)
    parser.add_argument("--min-match-count", type=int, default=5)
    parser.add_argument("--chunk-size", type=int, default=100000)
    parser.add_argument("--candidate-pool-size", type=int, default=400)
    parser.add_argument("--max-per-algorithm", type=int, default=16)
    parser.add_argument("--max-per-sky-region", type=int, default=20)
...
def apply_strict_mass_filters(audited: pd.DataFrame, min_match_count: int, max_nearest_sep_deg: float) -> pd.DataFrame:
    df = audited.copy()
    ...
    q_sep = pd.to_numeric(df["mass_nearest_sep_deg"], errors="coerce")
    q_cnt = pd.to_numeric(df["mass_match_count"], errors="coerce")
    q_score = pd.to_numeric(df["selection_score"], errors="coerce")
    df["mass_quality_score"] = (
        (1.0 / (1.0 + q_sep.fillna(np.inf) * 100.0))
        + np.log10(q_cnt.clip(lower=1).fillna(1.0))
        + q_score.fillna(0.0)
    )
    return df
...
def build_core_representatives(candidates: pd.DataFrame, preferred_algorithms: List[str], count: int, max_per_algorithm: int, max_per_sky_region: int) -> pd.DataFrame:
    ...
    core = add_input_metadata(core, "core_representative", "mass_augmented_candidate_selection_stricter")
...
def build_support_representatives(candidates: pd.DataFrame, preferred_algorithms: List[str], count: int, max_per_algorithm: int, max_per_sky_region: int) -> pd.DataFrame:
    ...
    support = add_input_metadata(support, "support_representative", "mass_augmented_candidate_selection_stricter")
...
def build_core_stacked(candidates: pd.DataFrame, preferred_algorithms: List[str], size: int) -> pd.DataFrame:
    ...
    core = add_input_metadata(core, "core_stacked", "mass_augmented_candidate_selection_stricter")
...
def build_support_stacked(candidates: pd.DataFrame, preferred_algorithms: List[str], size: int) -> pd.DataFrame:
    ...
    support = add_input_metadata(support, "support_stacked", "mass_augmented_candidate_selection_stricter")
...
def write_summary(
    ...
    lines = []
    lines.append("Cosmic Void final input builder summary (005 stricter plus diversity)")
    lines.append("===================================================================")
    ...
    lines.append("-------------------")
    lines.append(
        "This 005 stage tightens the matching defaults further and adds diversity-aware selection. "
        "A void must pass the strict mass criteria and is then chosen with simple limits per algorithm and sky region."
    )
...
def main() -> int:
    ...
    audited = apply_strict_mass_filters(audited, args.min_match_count, args.max_nearest_sep_deg)
