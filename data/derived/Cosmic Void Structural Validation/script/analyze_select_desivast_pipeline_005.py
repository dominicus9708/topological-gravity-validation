...
DERIVED_ROOT_DEFAULT = Path("data") / "derived" / "Cosmic Void Structural Validation" / "derived"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        default=str(DERIVED_ROOT_DEFAULT),
        help="Base derived root. Actual outputs will go under output-root/derived/YYYYMMDD_HHMMSS/",
    )
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite standardized CSV files inside the timestamp run folder")
    ...
    parser.add_argument("--run-tag", default="", help="Optional run tag appended after timestamp, e.g. 20260416_203000_test")
...
def make_run_output_dir(base_output_root: Path, run_tag: str) -> Path:
    timestamp = datetime.now().strftime(TIMESTAMP_FORMAT)
    folder_name = f"{timestamp}_{run_tag}" if run_tag else timestamp
    run_root = base_output_root / "derived" / folder_name
...
def infer_distance_fields(std: pd.DataFrame, algorithm: str) -> pd.DataFrame:
    out = std.copy()

    # Default coordinate radius proxy
    out["coord_r3d"] = np.where(
        out["coord_x"].notna() & out["coord_y"].notna() & out["coord_z"].notna(),
        np.sqrt(out["coord_x"] ** 2 + out["coord_y"] ** 2 + out["coord_z"] ** 2),
        np.nan
    )

    # Algorithm-specific conservative interpretation
    if algorithm in ("VIDE", "REVOLVER"):
        # Use explicit redshift only if values look physically plausible for catalog redshift.
        red = safe_numeric(out["redshift"])
        plausible = red.between(0, 1, inclusive="both")
        out["distance_proxy"] = np.where(plausible, red, np.nan)
        out["distance_kind"] = np.where(plausible, "trusted_redshift", "missing")
        out["distance_status"] = np.where(plausible, "usable", "missing_or_invalid")

        # fallback to coord_r3d only as proxy if no plausible redshift exists
        need_proxy = pd.isna(out["distance_proxy"]) & out["coord_r3d"].notna()
        out.loc[need_proxy, "distance_proxy"] = out.loc[need_proxy, "coord_r3d"]
        out.loc[need_proxy, "distance_kind"] = "coord_r3d_proxy"
        out.loc[need_proxy, "distance_status"] = "proxy_only"

    elif algorithm == "VoidFinder":
        # Treat VoidFinder distance only as geometric proxy for now
        out["distance_proxy"] = out["coord_r3d"]
        out["distance_kind"] = np.where(out["coord_r3d"].notna(), "coord_r3d_proxy", "missing")
        out["distance_status"] = np.where(out["coord_r3d"].notna(), "proxy_only", "missing")

    elif algorithm == "ZOBOV":
        # Be especially conservative: explicit redshift-like field can be invalid here.
        red = safe_numeric(out["redshift"])
        plausible = red.between(0, 1, inclusive="both")
        out["distance_proxy"] = np.nan
        out["distance_kind"] = "missing"
        out["distance_status"] = "missing"

        # Prefer plausible explicit redshift only
        out.loc[plausible, "distance_proxy"] = red[plausible]
        out.loc[plausible, "distance_kind"] = "trusted_redshift"
        out.loc[plausible, "distance_status"] = "usable"

        # Otherwise allow coord proxy, but only if its scale is not absurdly large
        proxy_ok = (~plausible) & out["coord_r3d"].notna() & (out["coord_r3d"] < 1e6)
        out.loc[proxy_ok, "distance_proxy"] = out.loc[proxy_ok, "coord_r3d"]
        out.loc[proxy_ok, "distance_kind"] = "coord_r3d_proxy"
        out.loc[proxy_ok, "distance_status"] = "proxy_only"

        # If redshift-like values are absurd, mark explicitly
        absurd_red = red.notna() & (~plausible)
        out.loc[absurd_red & out["distance_status"].eq("missing"), "distance_kind"] = "invalid_redshift_like"
        out.loc[absurd_red & out["distance_status"].eq("missing"), "distance_status"] = "invalid_redshift_like"
    ...
    out["has_distance_proxy"] = pd.notna(out["distance_proxy"])
    out["distance_rank_pct"] = safe_numeric(out["distance_proxy"]).rank(pct=True)
    return out
...
def standardize_catalog(df: pd.DataFrame, ctx: CatalogContext) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    standardized = infer_distance_fields(standardized, ctx.algorithm)
...
def summarize_catalog(df_raw: pd.DataFrame, df_std: pd.DataFrame, ctx: CatalogContext, matched: Dict[str, str]) -> Dict[str, object]:
    return {
        ...
        "distance_status_counts": str(df_std["distance_status"].value_counts(dropna=False).to_dict()),
...
def add_selection_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Distance is now diagnostic and lower-weighted
    ...
    # Quality bonus / penalty from distance status
    status_bonus = {
        "usable": 1.0,
        "proxy_only": 0.75,
        "missing_or_invalid": 0.30,
        "invalid_redshift_like": 0.10,
        "missing": 0.20,
    }
    out["score_distance_status"] = out["distance_status"].map(status_bonus).fillna(0.20)

    nm = safe_numeric(out["n_members"])
    out["score_members"] = nm.rank(pct=True) if nm.notna().sum() > 0 else 0.5

    out["selection_score"] = (
        0.40 * out["score_radius"]
        + 0.30 * out["score_edge"]
        + 0.15 * out["score_geometry"]
        + 0.05 * out["score_distance"]
        + 0.05 * out["score_distance_status"]
        + 0.05 * out["score_members"]
    )
...
def choose_representatives(
    ...
    # prefer usable/proxy_only, demote invalid distance cases
    priority = df.copy()
    status_order = {
        "usable": 0,
        "proxy_only": 1,
        ...
        "missing": 3,
        "invalid_redshift_like": 4,
    }
    priority["_status_order"] = priority["distance_status"].map(status_order).fillna(9)
    pool = priority.sort_values(
        ["_status_order", "selection_score", "radius_eff"],
    ...
    if "_status_order" in chosen.columns:
        chosen = chosen.drop(columns=["_status_order"])
...
def main() -> int:
    ...
    base_output_root = resolve_path(project_root, args.output_root)
    output_root = make_run_output_dir(base_output_root, args.run_tag)
    ...
    summary_lines.append(f"Base output root: {base_output_root}")
    summary_lines.append(f"Run output root: {output_root}")
    summary_lines.append("")
    ...
    if not representatives.empty:
        ...
        for _, row in representatives.iterrows():
            summary_lines.append(
                ...
                f"distance_status={row['distance_status']} :: selection_score={row['selection_score']:.4f}"
            )
        summary_lines.append("")
    ...
    summary_lines.append("-------------------")
    summary_lines.append(
        "005 writes each run into data/derived/Cosmic Void Structural Validation/derived/derived/YYYYMMDD_HHMMSS/. "
        "It also uses stricter algorithm-specific distance handling and keeps distance as a lower-weight diagnostic field."
    )
