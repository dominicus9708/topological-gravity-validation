...
def patch_local_version_file(repo_root: Path) -> None:
    version_file = repo_root / "galstreams" / "_version.py"
    if not version_file.exists():
        version_file.write_text('version = "0+local"\n', encoding="utf-8")


def patch_galstreams_core_for_gala_141(repo_root: Path) -> None:
    core_path = repo_root / "galstreams" / "core.py"
    text = core_path.read_text(encoding="utf-8")

    old_block = '''      if version.parse(gala.__version__)<=version.parse("1.4"):
    ...
    new_block = '''      if hasattr(gc.GreatCircleICRSFrame, "from_pole_ra0"):
         self.stream_frame = gc.GreatCircleICRSFrame.from_pole_ra0(
            pole=self.mid_pole,
            ra0=self.mid_point.icrs.ra,
            origin_disambiguate=self.mid_point.icrs
    ...
    elif 'GreatCircleICRSFrame.from_pole_ra0' in text and 'hasattr(gc.GreatCircleICRSFrame, "from_pole_ra0")' not in text:
        start = text.find('      if version.parse(gala.__version__)<=version.parse("1.4"):')
        end = text.find('      #Compute and store polygon vertices', start)
...
def verify_runtime_dependencies() -> None:
    missing = []
    for module_name in ["numpy", "scipy", "astropy", "gala", "matplotlib", "pandas", "packaging"]:
        try:
    ...
    if missing:
        ...
        raise RuntimeError(
            "Required runtime modules are missing in the current conda environment: "
            f"{missing_str}\n"
            "Install them in the active conda environment first, then rerun this script."
        )
...
def generate_topcat_csvs_from_source(repo_root: Path, output_root: Path) -> None:
    ...
    patch_galstreams_core_for_gala_141(repo_root)

    print("[INFO] Importing patched galstreams directly from local source snapshot")
