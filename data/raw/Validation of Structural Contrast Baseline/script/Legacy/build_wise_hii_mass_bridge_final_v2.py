...
WISE HII mass-bridge finalizer v2
...
def _join_source_rows(src_df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = src_df[cols].copy()
    for c in cols:
        work[c] = work[c].fillna("").astype(str)

    grouped_rows = []
    for wise_name, group in work.groupby("wise_name", sort=False):
        rows = []
        ...
        grouped_rows.append(
            {
                "wise_name": wise_name,
                "source_bundle": " || ".join(rows),
            }
        )

    return pd.DataFrame(grouped_rows, columns=["wise_name", "source_bundle"])
...
def main() -> int:
    ...
    print("=" * 72)
    print("Validation of Structural Contrast Baseline - mass bridge finalizer v2")
    print("=" * 72)
    print("[OK] Created:")
    print(final_path)
    print(manifest_path)
