...
"""
Apply FITS raw evidence rows into working FITS tables.
...
Evidence folder:
- data/raw/Validation of Structural Contrast Baseline/wise_hii_catalog/fits/raw/

Expected evidence schema
------------------------
Each CSV under fits/raw may contain columns such as:
source_key, wise_name, source_service, survey_name, band_name, ra, dec, cutout_radius_arcmin,
product_url, access_notes, download_status, local_path, fits_verified, image_plane_verified,
matching_notes

Purpose
-------
- collect FITS/source rows from fits/raw/*.csv
- append unique source rows into a working FITS source registry
- update per-target working FITS candidates using the best available row
- keep the original initial tables untouched

Outputs
-------
- wise_hii_fits_source_registry_working.csv
- wise_hii_fits_candidates_working.csv
- wise_hii_fits_working_manifest.txt
"""
...
def truthy(v) -> bool:
    ...
    if isinstance(v, bool):
        return v
    return str(v).strip().lower() in {"1", "true", "t", "yes", "y", "verified", "downloaded", "ok"}
    ...
    if not evidence.empty:
        ...
        for _, row in best_df.iterrows():
            ...
            if has_text(row.get("source_service")):
                working_candidates.loc[mask, "fits_source_service"] = row.get("source_service")
            if has_text(row.get("band_name")):
                working_candidates.loc[mask, "fits_band"] = row.get("band_name")
            if has_text(row.get("product_url")):
                working_candidates.loc[mask, "fits_url"] = row.get("product_url")
                working_candidates.loc[mask, "fits_downloadable"] = True
            if has_text(row.get("local_path")):
                working_candidates.loc[mask, "fits_local_path"] = row.get("local_path")
            if has_text(row.get("fits_verified")):
                working_candidates.loc[mask, "fits_local_verified"] = row.get("fits_verified")
            if has_text(row.get("image_plane_verified")):
                working_candidates.loc[mask, "fits_image_plane_verified"] = row.get("image_plane_verified")
