...
def cast_boollike_columns(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    for c in cols:
        if c in df.columns:
            # object로 강제해도 되고, pandas BooleanDtype을 써도 되지만
            # 여기서는 문자열/불리언 혼합 입력에도 안전하게 object로 둡니다.
            df[c] = df[c].astype("object")
    return df
...
def main() -> int:
    ...
    candidates = ensure_columns(candidates, TEXTLIKE_CANDIDATE_COLUMNS + BOOLEANLIKE_COLUMNS)
    candidates = cast_textlike_columns(candidates, TEXTLIKE_CANDIDATE_COLUMNS)
    candidates = cast_boollike_columns(candidates, BOOLEANLIKE_COLUMNS)
    ...
    working_candidates = cast_boollike_columns(working_candidates, BOOLEANLIKE_COLUMNS)
    ...
    lines = [
        ...
        "String-like and bool-like update columns are explicitly cast before assignment.",
