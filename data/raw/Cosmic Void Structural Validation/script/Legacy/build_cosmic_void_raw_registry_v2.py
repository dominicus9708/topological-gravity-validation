...
"""
build_cosmic_void_raw_registry_v2.py

Purpose
-------
Create and maintain the raw-data registry structure for:

    data/raw/Cosmic Void Structural Validation/

This script does NOT pretend that documentation URLs are direct data-download URLs.
Instead, it creates a clean raw-data structure, copies registry CSVs into the correct
location, generates dataset subfolders and README files, and writes helper files for
manual download / later downloader extension.

Why this exists
---------------
The previous scaffold could create folders and README files, but its output root was
not aligned with the project rule. This version fixes the root to:

    data/raw/Cosmic Void Structural Validation/

and keeps every generated artifact inside that tree.
"""
...
def main() -> int:
    ...
    datasets_csv = (project_root / args.datasets_csv).resolve() if not Path(args.datasets_csv).is_absolute() else Path(args.datasets_csv)
    sources_csv = (project_root / args.sources_csv).resolve() if not Path(args.sources_csv).is_absolute() else Path(args.sources_csv)
    target_root = (project_root / args.target_root).resolve() if not Path(args.target_root).is_absolute() else Path(args.target_root)
    ...
    # Copy local registry CSVs into target raw root.
    ...
    # Also keep original filenames if the user prefers those.
    ...
    # Save expanded CSVs inside the correct target root.
